 ...
 */
public class PayPalResponseConverter extends
		AbstractPopulatingConverter<AbstractResponseType, AbstractResultData> {
	 ...
	 */
	@Override
	public void populate(AbstractResponseType response,
		...
		if (CollectionUtils.isNotEmpty(errors)) {
			for (ErrorType error : errors) {
				...
				errorMessageBuilder.append(error.getErrorCode())
						.append(PaypalConstants.ERROR_MESSAGE_CODE_SEPARATOR);
				errorMessageBuilder.append(error.getShortMessage())
						.append(PaypalConstants.ERROR_MESSAGE_SHORT_MESSAGE_SEPARATOR);
				errorMessageBuilder.append(error.getLongMessage());
				errorMessagesList.add(errorMessageBuilder.toString());
				String severityCodeValue = error.getSeverityCode()
						.value()
						.toUpperCase();
	...
	@Override
	protected AbstractResultData createTarget() {
		return new AbstractResultData();
	}
}
