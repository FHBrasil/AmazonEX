...
public class PayPalPaymentFacade extends DefaultPaymentFacade {
     ...
     * @return
     */
    public SetExpressCheckoutResultData preparePaypalPayment(
            final SetExpressCheckoutRequestData requestData) {
        LOG.debug("Prepare payment for PayPal");
        final SetExpressCheckoutRequestType request =
                (SetExpressCheckoutRequestType) setExprCheckoutReqDataConverter.convert(requestData);
        final SetExpressCheckoutResponseType response =
                paypalPaymentService.setExpressCheckout(request);
        final SetExpressCheckoutResultData resultData =
                (SetExpressCheckoutResultData) setExprCheckoutResConverter.convert(response);
        if (PaypalConstants.STATUS_SUCCESS.equalsIgnoreCase(resultData.getAck())) {
            List<PaymentDetailsType> paymentDetails =
                    request.getSetExpressCheckoutRequestDetails().getPaymentDetails();
            final String paymentAction =
                    CollectionUtils.isNotEmpty(paymentDetails) ? paymentDetails.get(0)
                            .getPaymentAction().value() : configurationService.getConfiguration()
                            .getString(PaypalConstants.DEFAULT_PAYMENT_ACTION_NAME);
            ...
            if (Boolean.TRUE.toString().equalsIgnoreCase(
                    configurationService.getConfiguration().getString(
                            PaypalConstants.USE_REFERENCE_TRANSCATION))) {
    ...
    public GetExpressCheckoutDetailsResultData getExpressCheckoutDetails(
        ...
        GetExpressCheckoutDetailsRequestType request =
                (GetExpressCheckoutDetailsRequestType) getExprCheckoutDetReqDataConverter
                        .convert(requestData);
        final GetExpressCheckoutDetailsResponseType response =
                paypalPaymentService.getExpressCheckoutDetails(request);
        GetExpressCheckoutDetailsResultData resultData =
                (GetExpressCheckoutDetailsResultData) getExprCheckoutDetResConverter
                        .convert(response);
        if (PaypalConstants.STATUS_SUCCESS.equalsIgnoreCase(resultData.getAck())) {
            final CartModel cart = cartService.getSessionCart();
            if (cart.getPaymentInfo() instanceof PaypalPaymentInfoModel) {
                final PaypalPaymentInfoModel paymentInfo =
                        (PaypalPaymentInfoModel) cart.getPaymentInfo();
                ...
                if (resultData.getBillingAddress() != null) {
                    ...
                    billingAddress =
                            addressReverseConverter.convert(resultData.getBillingAddress(),
                                    billingAddress);
                    final UserModel user = cart.getUser();
                    if (user instanceof CustomerModel
                            && ((CustomerModel) user).getType() == CustomerType.GUEST) {
                        billingAddress.setEmail(user.getUid().substring(
                                user.getUid().indexOf("|") + 1));
                    } else {
    ...
    /**
     * Make call to service's doExpressCheckoutPayment method. Additionally converts from dto
     * object to request and from
     ...
     */
    public DoExpressCheckoutPaymentResultData doExpressCheckoutPayment(
            final DoExpressCheckoutPaymentRequestData requestData) {
        final DoExpressCheckoutPaymentRequestType request =
                (DoExpressCheckoutPaymentRequestType) doExprCheckoutPaymentReqDataConverter
                        .convert(requestData);
        final DoExpressCheckoutPaymentResponseType response =
                paypalPaymentService.doExpressCheckoutPayment(request);
        DoExpressCheckoutPaymentResultData resultData =
                (DoExpressCheckoutPaymentResultData) doExprCheckoutPaymentResConverter
                        .convert(response);
        return resultData;
    }
    ...
    /**
     * Make call to service's authorization method. Additionally converts from dto object to
     * request and from response to
     * result dto object.
     *
     * @param requestData
     *            dto object with service call params
    ...
    public DoAuthorizationResultData doAuthorization(final DoAuthorizationRequestData requestData) {
        ...
        final DoAuthorizationRequestType request =
                (DoAuthorizationRequestType) doAuthorizationReqDataConverter.convert(requestData);
        final DoAuthorizationResponseType response = paypalPaymentService.doAuthorization(request);
        final DoAuthorizationResultData resultData =
                (DoAuthorizationResultData) doAuthorizationResConverter.convert(response);
        return resultData;
    }
    
    
    /**
     * Make call to service's capture method. Additionally converts from dto object to request and
     * from response to
     * result dto object.
     *
     * @param requestData
    ...
    public DoCaptureResultData doCapture(final DoCaptureRequestData requestData) {
        ...
        final DoCaptureRequestType request =
                (DoCaptureRequestType) doCaptureReqDataConverter.convert(requestData);
        final DoCaptureResponseType response = paypalPaymentService.doCapture(request);
        final DoCaptureResultData resultData =
                (DoCaptureResultData) doCaptureResConverter.convert(response);
        return resultData;
    }
     ...
     * @return
     */
    public DoReferenceTransactionResultData doReferenceTransaction(
            final DoReferenceTransactionRequestData requestData) {
        LOG.debug("DoReferenceTransaction call with PayPal");
        final DoReferenceTransactionRequestType request =
                (DoReferenceTransactionRequestType) doRefTransactionReqDataConverter
                        .convert(requestData);
        final DoReferenceTransactionResponseType response =
                paypalPaymentService.doReferenceTransaction(request);
        final DoReferenceTransactionResultData resultData =
                (DoReferenceTransactionResultData) doRefTransactionResConverter.convert(response);
        return resultData;
    }
    ...
    public String getFullResponseUrl(final String responseUrl, final boolean isSecure) {
        ...
        final String fullResponseUrl =
                getSiteBaseUrlResolutionService().getWebsiteUrlForSite(currentBaseSite, isSecure,
                        responseUrl);
    ...
    public
            void
            setSetExprCheckoutReqDataConverter(
    ...
    public
            void
            setGetExprCheckoutDetReqDataConverter(
    ...
    public
            void
            setGetExprCheckoutDetResConverter(
    ...
    public
            void
            setDoExprCheckoutPaymentReqDataConverter(
    ...
    public
            void
            setDoExprCheckoutPaymentResConverter(
    ...
    public
            void
            setDoAuthorizationReqDataConverter(
    ...
    @Required
    public void setDoCaptureReqDataConverter(
            final Converter<AbstractRequestData, AbstractRequestType> doCaptureReqDataConverter) {
        this.doCaptureReqDataConverter = doCaptureReqDataConverter;
    }
    
    
    @Required
    public void setDoCaptureResConverter(
            final Converter<AbstractResponseType, AbstractResultData> doCaptureResConverter) {
    ...
    public
            void
            setDoRefTransactionReqDataConverter(
                    final Converter<AbstractRequestData, AbstractRequestType> doRefTransactionReqDataConverter) {
    ...
    public void setDoRefTransactionResConverter(
            final Converter<AbstractResponseType, AbstractResultData> doRefTransactionResConverter) {
