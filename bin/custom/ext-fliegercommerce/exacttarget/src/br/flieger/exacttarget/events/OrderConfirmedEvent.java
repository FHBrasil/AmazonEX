...
public class OrderConfirmedEvent extends AbstractExacttargetOrderEvent
{
	@Resource
	...
	{
		final Subscriber[] superSubscribers = super.getSubscribers();
		final Subscriber[] newSubscribers = new Subscriber[superSubscribers.length * 2];
		
		final String sacEmail = Config.getParameter("exacttarget.sac.email");
		
		int i = 0;
		for (Subscriber subscriber : superSubscribers)
		{
			newSubscribers[i] = subscriber;
			i++;
			
			final Subscriber s = new Subscriber();
			s.setSubscriberKey(sacEmail);
			s.setEmailAddress(sacEmail);
			s.setAttributes(subscriber.getAttributes());
