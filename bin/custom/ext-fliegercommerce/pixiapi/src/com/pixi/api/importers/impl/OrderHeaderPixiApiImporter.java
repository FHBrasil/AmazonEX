 ...
 * @author jfelipe
 */
public class OrderHeaderPixiApiImporter extends AbstractPixiApiImporter {
    
    private static final Logger LOG = Logger
            .getLogger(OrderHeaderPixiApiImporter.class);
    //
     ...
     * 
     */
    public OrderHeaderPixiApiImporter() {
        super();
    ...
    /**
     * Imports the order header bins from Pixi. <br/>
     * 
     ...
     * @return A List of {@link OrderHeaderResult} containing the results from
     *         Pixi API
     ...
     */
    @Override
    public List<OrderHeaderResult> importData(
        ...
        return importOrderHeader(functionParameters);
    }
     ...
     * @throws KeyManagementException
     */
    private List<OrderHeaderResult> importOrderHeader(
            List<PixiFunctionParameter> functionParameters) {
        List<OrderHeaderResult> results = new ArrayList<OrderHeaderResult>();
        OrderHeaderResult result = null;
        checkValidParameters(functionParameters);
        SOAPMessage request;
        try {
            request = getPixiSoapApi().buildMessage(
                    PixiFunction.GET_ORDER_HEADER, functionParameters);
            SOAPMessage response;
            response = getPixiSoapApi().sendPixiWebServiceRequest(
                    request);
            ...
            if (responseXml != null) {
                ...
                if (rowTag != null) {
                    result = new OrderHeaderResult();
                    for (int i = 0; i < rowTag.getChildNodes().getLength(); i++) {
                        Node currentTag = rowTag.getChildNodes().item(i);
