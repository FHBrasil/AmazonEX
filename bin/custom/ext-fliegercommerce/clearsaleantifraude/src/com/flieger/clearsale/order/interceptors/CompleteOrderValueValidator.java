...
//o adyenservice sempre deve devolver sucesso, se o pedido já foi capturado ou pôde ser capturado nesta tentativa
    private boolean capturePayment(final OrderModel order)
    {
        ...
        {
            final PaymentTransactionEntryModel capture = 
                    paymentService.capture(transaction);
            
            //if the capture was received, it means that Adyen received the request for capture, if the capture was accepted, it means that the user account was charged 
            result &= 
                    capture != null 
                    && (AdyenTransactionStatus.CAPTURE_RECEIVED.equals(capture.getTransactionStatus()) 
                            || TransactionStatus.ACCEPTED.name().equals(capture.getTransactionStatus()));
        }           
        return result;
    }
}