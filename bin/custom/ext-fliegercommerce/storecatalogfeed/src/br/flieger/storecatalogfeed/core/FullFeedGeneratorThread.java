...
public abstract class FullFeedGeneratorThread extends AbstractFeedThread
{
	...
	public void run() 
	{
		...
		//launching all threads with the specified barrier to synchronize the call back code
		//that must be executed when all threads are finished.
		//
		CyclicBarrier barrier = getBarrier(totalThreads + 1); // total exporter threads plus current thread

		List<PartialFeedGeneratorThread> threads = new ArrayList<PartialFeedGeneratorThread>();

		//this variable decreases accordingly to the created threads, the initial value is the total amount of needed threads
		threadGroup: while(totalThreads > 0) {

			//while there's a need for more threads, and there are less then 6 threads already running, then we create/start more
			while((threads.size() < 6 && totalThreads > 0)) 
			{
				PartialFeedGeneratorThread thread = createPartialFeedGeneratorThread(barrier, (totalThreads--));
			...
			//remove finished threads from the list, so we are able to create/start more threads
			for(PartialFeedGeneratorThread thread : new ArrayList<PartialFeedGeneratorThread>(threads)) {
	 ...
	 * @param totalProducts The counter of all products that will be exported
	 ...
	 * @return
	 */
	protected PartialFeedGeneratorThread createPartialFeedGeneratorThread(CyclicBarrier barrier, int threadNr)
	{
