...
{
	private static final Logger LOG = Logger.getLogger(ExportStoreFeedJobPerformable.class.getName());
	...
	public PerformResult perform(CronJobModel cronjob)
	{
		if(!(cronjob instanceof ExportStoreFeedJobModel)) {
			throw new InvalidParameterException(cronjob.getCode() + " is not valid, class: " + cronjob.getClass().getSimpleName());
		}
		
		LOG.info("Started:" + new Date());

		Collection<String> templates = Config.getParametersByPattern("export.template.code").values();
		...
		if(!templates.isEmpty()) 
		{
			...
			for(final String template : templates) 
			{
				try 
				{
					FullFeedGeneratorThread handlerThread = createFullFeedGeneratorThread(cv, template);
	 ...
	 * @return
	 */
	private FullFeedGeneratorThread createFullFeedGeneratorThread(final CatalogVersionModel cv, final String template)
	{
