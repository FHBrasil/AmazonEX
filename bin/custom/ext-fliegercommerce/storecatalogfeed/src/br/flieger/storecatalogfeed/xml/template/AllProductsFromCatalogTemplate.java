...
public class AllProductsFromCatalogTemplate implements XMLTemplate<ProductModel>
{
	...
	protected static final DecimalFormat decimalFormat = new DecimalFormat("#0.00", new DecimalFormatSymbols(Locale.ENGLISH));
	...
	public Document apply(Set<ProductModel> products) {
		...
		for (final ProductModel product : products) {
			try {
				...
				Integer installment = price == null ? new Integer(0)
						: new Integer(calculateAvailableInstallments(
								price.doubleValue()));
				...
				rootElement.addContent(xmlUtils.createElement("url",
						getUrl(product), false));
				rootElement.addContent(xmlUtils.createElement("code", 
						product.getCode(), false));
				rootElement.addContent(xmlUtils.createElement("baseProductCode", 
						getBaseProduct(product).getCode(), false));
				rootElement.addContent(xmlUtils.createElement("name",
				...
				rootElement.addContent(xmlUtils.createElement("image",
				...
				rootElement.addContent(xmlUtils.createElement("parcelQty",
						installment.toString(), false));
				rootElement.addContent(xmlUtils.createElement("parcelAmount",
						formatPrice(getParcelAmount(price, BigDecimal.valueOf(
								installment.intValue()))),
						false));
				rootElement.addContent(xmlUtils.createElement("manufacturer",
						product.getManufacturerName(), false));
				rootElement.addContent(xmlUtils.createElement("ean", 
						product.getEan(), false));
				rootElement.addContent(xmlUtils.createElement("gender", 
						getGender(product), false));
				...
				rootElement.addContent(xmlUtils.createElement("size",
						getSize(product), false));
				rootElement.addContent(xmlUtils.createElement("color",
						getColor(product), false));
				rootElement.addContent(xmlUtils.createElement("weight",
						getWeight(product).toString(), false));
				rootElement.addContent(xmlUtils.createElement("sale_price",
						getSalePrice(product), false));
				...
				final Element additionalImagesElement = new Element(
						"additionalImages");
				for(MediaModel additionalImage: getAdditionalImages(product)) {
					Element imageElement = new Element("image");
					imageElement.addContent(xmlUtils.createElement(
							"additionalUrl", getImageUrl(additionalImage), false));
					imageElement.addContent(xmlUtils.createElement(
							"additionalImageDimensions",
	 ...
	 */
	protected void addStoreElements(XMLUtils xmlUtils, final Element rootElement, CatalogVersionModel catalogVersion)
	{
	...
	protected boolean isBaseProduct(final ProductModel product) {
		return (product instanceof VariantProductModel 
			...
			|| product instanceof HeringProductModel;
	}
	...
	protected ProductModel getBaseProduct(final ProductModel product) {
		return product instanceof HeringProductModel
				? product
	...
	protected String getFormattedModifiedTime(final ProductModel product) {
		...
		return modifiedtime != null 
				? GOOGLE_DF.format(modifiedtime) + "T00:00:00+00:00"
				: "";
	}
	...
	protected String getImageUrl(final ProductModel product) {
		if (product instanceof HeringSizeVariantProductModel) {
			return getImageUrl(((HeringSizeVariantProductModel) product)
					.getBaseProduct().getPicture());
		}
	...
	protected String getImageDimensions(final MediaModel image) {
		if(image != null && image.getMediaFormat() != null 
			...
			return image.getMediaFormat().getQualifier()
					.replaceAll("(?i)w", "").replaceAll("(?i)h", "");
		}
		return "";
	}
	...
	protected BigDecimal getParcelAmount(BigDecimal price, BigDecimal parcelQty) {
		return price == null || price.doubleValue() == 0.0 ? null 
				: price.divide(parcelQty, PRICE_SCALE, PRICE_ROUND_MODE);
	}
	...
	protected BigDecimal getPrice(final Collection<PriceRowModel> prices) {
		if (CollectionUtils.isNotEmpty(prices)) {
			...
			for (; priceIterator.hasNext();)	{
				...
				BigDecimal price = BigDecimal.valueOf(actualPrice.getPrice()
						.doubleValue());
				return price;
			}
		}
	...
	protected BigDecimal getOldPrice(final ProductModel product) {
		return product.getOldPrice() != null ? product.getOldPrice()
				.setScale(PRICE_SCALE, PRICE_ROUND_MODE) : null;
	}
	...
	protected String getUrl(final ProductModel product) {
		...
		final String urlForSite = siteBaseUrlResolutionService
				.getWebsiteUrlForSite(site, false, productURL);
		return urlForSite;
	}
	...
	protected String getGender(final ProductModel product) {
		String genders = "";
		HeringProductModel heringProduct = (HeringProductModel) 
	...
	{
		if(product instanceof HeringSizeVariantProductModel) { 
			return ((HeringSizeVariantProductModel) product).getColorFullDescription() != null
					? ((HeringSizeVariantProductModel) product).getColorFullDescription()
							.replace(" ", "/")
					: "";
		}
		return "";
	...
	protected Integer getVariantStock(final ProductModel product, BaseStoreModel store)
	{
		if(commerceStockService.isStockSystemEnabled(store)) {
			return new Integer(commerceStockService.getStockLevelForProductAndBaseStore(product, store).intValue());
		} else {
	...
	public Integer getProductTotalStock(final ProductModel product) {
		...
		for(VariantProductModel variant: variants) {
			Collection<VariantProductModel> variantsSecondLevel = variant
					.getVariants();
	...
	{
		if(product instanceof HeringSizeVariantProductModel) {
			return decimalFormat.format(((HeringSizeVariantProductModel) product)
					.getWeight());
		}
		return "";
	}
	...
	{
		PriceRowModel priceModel = product.getEurope1Prices().iterator().hasNext()
				? product.getEurope1Prices().iterator().next() : null;
	...
	protected String getSaleDateRange(final ProductModel product)
	{
		...
		StandardDateRange dateRange = product.getEurope1Prices().iterator()
				.hasNext() ? 
						product.getEurope1Prices().iterator().next().getDateRange()
						: null;
		return dateRange != null ?
				dateFormat.format(dateRange.getStart()) + "T"
	 ...
	 * @return
	 */
	protected Set<String> getAllVariantsAvailableColors(
			final ProductModel product) {
		Set<String> colors = new HashSet<String>();
		List<HeringStyleVariantProductModel> variantStyles = 
				new ArrayList<HeringStyleVariantProductModel>();
		if(product instanceof HeringProductModel) {
			variantStyles = variantsUtils.getAvailableStyleVariants(
					(HeringProductModel) product);
	...
	protected BigDecimal getFirstVariantPrice(final ProductModel product) {
		...
		for(VariantProductModel variant: variants) {
			Collection<VariantProductModel> variantsSecondLevel = variant
					.getVariants();
			for(VariantProductModel variantSecondLevel : variantsSecondLevel) {
   			price = variantSecondLevel.getEurope1Prices().iterator().hasNext()
   					? variantSecondLevel.getEurope1Prices().iterator().next()
   							.getPrice()
   					: null;
	...
	protected int calculateAvailableInstallments(final double price) {
		if (price < 60) return 1;
		if (price >= 60.00 && price < 90.00) return 2;
		if (price >= 90.00 && price < 120.00) return 3;
		if (price >= 120.00 && price < 150.00) return 4;
		return 5;
	}
