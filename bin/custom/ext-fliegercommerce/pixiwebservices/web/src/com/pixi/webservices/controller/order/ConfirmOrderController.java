...
public class ConfirmOrderController extends AbstractPixiSecuredController
{
     ...
     * This method just confirms the received order, the code to export is within the class
     * {@link ExportOrdersController}
     * 
     * @param orderId
     *            Order id to be marked as exported
     * @param session
     * @return
     */
    @RequestMapping(method = RequestMethod.GET, produces = "text/xml", params = "action=" + ACTION)
    public @ResponseBody ImportOrderStatusResponse handle(
            ...
            @RequestParam final String session)
    {
        ...
        // XXX estamos usando getOrderDetailsForGUID para compilar na 5.1.1.7, precisa validar se
        // funciona
        // final OrderModel order = getCustomerAccountService().getOrderDetailsForGUID(orderId,
        // store);
        // FIXME: Isso é só um teste. Ver qual o service correto a se chamar para buscar uma
        // order.
        final OrderModel order = pixiOrderService.getOrderForCodeAndStore(orderId, store);
        if (order == null)
        {
    ...
    private ImportOrderStatusResponse getResponse(final String orderId, final String session,
