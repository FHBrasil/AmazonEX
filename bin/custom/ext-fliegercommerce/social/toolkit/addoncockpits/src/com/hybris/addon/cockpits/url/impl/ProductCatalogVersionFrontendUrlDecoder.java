...
import com.hybris.addon.common.url.impl.BaseFrontendRegexUrlDecoder;
...
public class ProductCatalogVersionFrontendUrlDecoder<ProductModel> extends BaseFrontendRegexUrlDecoder
{
	...
	protected ProductModel translateId(final String id)
	{
		...
		final SearchResult<ProductModel> result = getFlexibleSearchService().search(
				"select {PK} from {Product} where {code} = ?id and {catalogVersion} in (?catalogVersions)", params);
