 ...
 *
 */
public abstract class BeforeViewJsPropsHandlerAdaptee implements
	...
	/**
	 * Custom variable node name. If you do not want to use name in JS_VARIABLES_MODEL_NAME, this custom name
	 * will be used.
	 */
	...
	@Override
	public String beforeView(final HttpServletRequest request,
	...
	/**
	 * Method create Map of JavaScript variables. Format of map:
	 * index => addOn name
	 * value => List<JavaScriptVariableData>
	 * @param model
	 */
	@SuppressWarnings("unchecked")
	protected void attachJSVariablesToModel(final ModelMap model) {
		if(model!=null) {
			Map<String, List<JavaScriptVariableData>> jsVariables = (Map<String, List<JavaScriptVariableData>>)model.get(detectJsModelName());
			if(jsVariables==null) {
			...
			final List<JavaScriptVariableData> jsVariablesList = JavaScriptVariableDataFactory.createFromMap(((JavaScriptMessageResourcesAccessor)getMessageSource()).getAllMessages(currentLocale));
	 ...
	 */
	public abstract String beforeViewJsProps(final HttpServletRequest request,
			final HttpServletResponse response, final ModelMap model, final String viewName);
	...
	/**
	 * @param addonResourceBundleSource the addonResourceBundleSource to set
	 */
	@Required
	...
	/**
	 * @param jsVariableModelName the jsVariableModelName to set
	 */
