...
import com.hybris.addon.common.setup.AddOnSampleDataImportService;
...
public class DefaultAddonSampleDataImportService extends AbstractSystemSetup implements
	...
	@Override
	public void importSampleData(final String extensionName, final SystemSetupContext context, final List<ImportData> importData, final boolean solrReindex) {
		
		if (getBooleanSystemSetupParameter(context, IMPORT_SAMPLE_DATA))
		{
			importCommonData(context, extensionName);
			
			for (ImportData importd : importData)
			{
			...
			for (ImportData importd : importData)
			{
				for (String contentCatalogName : importd.getContentCatalogNames())
				{
			...
			for (ImportData importd : importData)
			{
				for (String storeName : importd.getStoreNames())
				{
					importStoreLocations(context, extensionName, storeName);

				}
				
				importStoreInitialData(context, extensionName, importd.getStoreNames(), importd.getProductCatalogName(), importd.getContentCatalogNames(), solrReindex);
	...
	protected void importProductCatalog(final SystemSetupContext context, final String importDirectory, final String catalogName)
	{
		...
		// Load Products
		importImpexFile(context, "/" + importDirectory + "/import/productCatalogs/" + catalogName + "ProductCatalog/products.impex", false);
		importImpexFile(context, "/" + importDirectory + "/import/productCatalogs/" + catalogName
		...
		importImpexFile(context, "/" + importDirectory + "/import/productCatalogs/" + catalogName
						+ "ProductCatalog/users.impex", false);
	...
	protected void importStoreInitialData(final SystemSetupContext context, final String importDirectory, final List<String> storeNames,
			final String productCatalog, final List<String> contentCatalogs, boolean solrReindex)
	{
		final String importRoot = "/" + importDirectory + "/import";
