...
/**
 * Extends the storefront CMS Component Registry so AddOns can easily declare additional CMS Component Renderer Mappings by
 * registring existing beans of type AddOnCMSComponentRendererMapping.
 *
 */
public class AddOnCMSComponentRendererRegistry extends DefaultCMSComponentRendererRegistry implements InitializingBean, ApplicationContextAware {
	...
	public void afterPropertiesSet() throws Exception {
		...
		final Map<String,AddOnCMSComponentRendererMapping> mappings = this.applicationContext.getBeansOfType(AddOnCMSComponentRendererMapping.class);
		if (mappings != null && mappings.size() > 0)
		{
			for (String key : mappings.keySet())
			{
				final AddOnCMSComponentRendererMapping mapping = mappings.get(key);
				if (validate)
				{
