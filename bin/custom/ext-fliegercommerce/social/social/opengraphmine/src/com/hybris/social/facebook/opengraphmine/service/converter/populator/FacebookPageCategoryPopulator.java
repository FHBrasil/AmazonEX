 ...
 * 
 */
public class FacebookPageCategoryPopulator implements Populator<Page, FacebookPageModel>
{
	private static final Logger LOG = Logger.getLogger(FacebookPageCategoryPopulator.class);

	private PathMatcher pathMatcher;
	private String pathMatchPattern = "/**/c/{code}";
					...
					getSessionService().executeInLocalView(new SessionExecutionBody()
					{
						...
						public void executeWithoutResult()
						{
							...
							try
							{
								...
								if (!target.getLinkedCategories().contains(category))
								{
									final Set<CategoryModel> categories = new LinkedHashSet<CategoryModel>(target.getLinkedCategories());
									categories.add(category);
									target.setLinkedCategories(categories);
								}
							}
							catch (final UnknownIdentifierException e)
							{
								LOG.error("unable to locate visible category for code " + code);
							}
