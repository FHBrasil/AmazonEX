        ...
        implements AdyenPaymentService
{
     ...
     * (non-Javadoc)
     * @see
     * de.hybris.platform.payment.impl.DefaultPaymentServiceImpl#capture(de.hybris.platform.payment
     * .model.
            ...
            throws AdapterException
    {
        ...
        final AdyenCardPaymentService cardPaymentService =
                (AdyenCardPaymentService) this.getCardPaymentService();
        ...
        PaymentTransactionEntryModel thisCapture =
                (PaymentTransactionEntryModel) this.modelService
                        .create(PaymentTransactionEntryModel.class);
        // se houve uma <b>tentativa</b> de captura que não teve sucesso, tenta novamente, senão
        // retorna a captura que já foi feita
        if (captureEntry != null)
        {
            if (TransactionStatus.ERROR.name().equals(captureEntry.getTransactionStatus()))
            {
                if (TransactionStatusDetails.COMMUNICATION_PROBLEM.name().equals(
                        captureEntry.getTransactionStatus()))
                {
                    thisCapture = captureEntry;
                    captureCode = captureEntry.getCode();
                }// se não é communication problem, então pode ser Insufficient balance on payment
                 // ou Operation requested after maximum period allowed: 5 days
                else
                {
        ...
        thisCapture.setTime(captureResult.getRequestTime() == null ? new Date() : captureResult
                .getRequestTime());
    ...
    /*
     * (non-Javadoc)
     * @see de.hybris.platform.payment.impl.DefaultPaymentServiceImpl#authorize(java.lang.String,
     * java.math.BigDecimal,
     * java.util.Currency, de.hybris.platform.core.model.user.AddressModel, java.lang.String,
     * java.lang.String,
     * java.lang.String)
     */
    @Override
            ...
            final String merchantAccount) throws AdapterException
    {
        ...
        return authorizeCardInternal(transaction, amount, currency, shippingInfo, subscriptionID,
            ...
            throws AdapterException
    {
        ...
        AdyenCardPaymentService cardPaymentService =
                (AdyenCardPaymentService) this.getCardPaymentService();
        ...
        Short installment =
                new Short(((CreditCardPaymentInfoModel) cart.getPaymentInfo()).getInstallment()
                        .toString());
        ...
        if (TransactionStatusDetails.SUCCESFULL.equals(result.getTransactionStatusDetails())) {
            if (result.getCurrency() != null) {
                entry.setCurrency(this.commonI18NService.getCurrency(result.getCurrency()
                        .getCurrencyCode()));
            }
            entry.setType(PaymentTransactionType.AUTHORIZATION);
            entry.setTime(result.getAuthorizationTime() == null ? new Date() : result
        ...
        else
        {
            ...
            LOG.info("Status desconhecido ao autorizar o cartão pelo Adyen. Status --> "
     ...
     * (non-Javadoc)
     * @see de.hybris.platform.payment.impl.DefaultPaymentServiceImpl#authorize(java.lang.String,
     * java.math.BigDecimal, java.util.Currency, de.hybris.platform.core.model.user.AddressModel,
     * java.lang.String)
     */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * com.adyen.services.payment.AdyenPaymentService#authorizeDebit(com.flieger.payment.model
     * .HeringDebitPaymentInfoModel, java.lang.String)
     */
    @Override
    public PaymentTransactionEntryModel authorizeDebit(
            ...
            final String paymentProvider, final String merchantAccount)
    {
        ...
        debitPaymentInfoModel.setReturnUrl(debitPaymentInfoModel.getReturnUrl() + "?paymentInfoPK="
                + debitPaymentInfoModel.getPk());
            ...
            final String merchantAccount)
    {
        ...
        {
            if (result.getCurrency() != null) {
                transactionEntry.setCurrency(this.commonI18NService.getCurrency(result.getCurrency()
                        .getCurrencyCode()));
            }
            transactionEntry.setAmount(result.getTotalAmount());
            transactionEntry.setType(PaymentTransactionType.AUTHORIZATION);
            transactionEntry.setTime(result.getAuthorizationTime() == null ? new Date() : result
                    .getAuthorizationTime());
            ...
            transactionEntry.setTransactionStatusDetails(result.getTransactionStatusDetails()
                    .toString());
        ...
        else
        {
            ...
            // SPJ Acrescentada informa��o ao log para mapeamento do erro emprodu��o 16-06-2014
            // LOG.error("Status desconhecido ao autorizar o boleto pelo Adyen.");
            LOG.info("Status desconhecido ao autorizar o debito pelo Adyen. Status retornado -->"
                    + result.getTransactionStatusDetails());
            return null;
        }
    ...
    private CardInfo createCardInfo()
    {
        ...
        CreditCardPaymentInfoModel paymentInfo =
                (CreditCardPaymentInfoModel) getCartService().getSessionCart().getPaymentInfo();
