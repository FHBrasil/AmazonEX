...
//			LOG.info(payResult.getPaRequest());
			...
			if(payResult.getAdditionalData() != null) {
				...
				for(AnyType2AnyTypeMapEntry entry : payResult.getAdditionalData()){
					if(!StringUtils.isEmpty(entry.getKey().toString()) && entry.getKey().equals("refusalReasonRaw")) {
					...
					&& (today.get(2) > request.getCard().getExpirationMonth().intValue()))
			{
				result.setTransactionStatus(TransactionStatus.REJECTED);
				result.setTransactionStatusDetails(TransactionStatusDetails.INVALID_CARD_EXPIRATION_DATE);
			} else if (payResult.getResultCode() != null && payResult.getResultCode().equals(AdyenTransactionStatus.AUTHORIZED))
			{
				result.setTransactionStatus(TransactionStatus.ACCEPTED);
				result.setTransactionStatusDetails(TransactionStatusDetails.SUCCESFULL);
			} else if (payResult.getResultCode() != null && payResult.getResultCode().equals(AdyenTransactionStatus.RECEIVED))
			{
				result.setTransactionStatus(TransactionStatus.ACCEPTED);
				result.setTransactionStatusDetails(TransactionStatusDetails.SUCCESFULL);
			} else
		...
		catch (Exception ex)
		{
			...
			LOG.error("Ocorreu um erro na autorização do cartão pelo Adyen ["+ex.getMessage()+"]", ex);
			result.setTransactionStatus(TransactionStatus.ERROR);
