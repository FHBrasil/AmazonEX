package com.fliegersoftware.addon.paypal.controllers.utils;
...
public class PayPalUserHelper {
    ...
    public boolean isHardLogin(final Model model) {
        HttpServletRequest request =
                (HttpServletRequest) model.asMap().get(REQUEST_MODEL_ATTRIBUTE_NAME);
    ...
    public boolean isHardLogin(HttpServletRequest request, boolean ignoreCookie) {
        ...
        if (((!userFacade.isAnonymousUser() || isAnonymousCheckout()) && (checkForGUIDCookie(
                request, guid) || ignoreCookie))) {
            isHardLogin = true;
        }
        return isHardLogin;
    }
