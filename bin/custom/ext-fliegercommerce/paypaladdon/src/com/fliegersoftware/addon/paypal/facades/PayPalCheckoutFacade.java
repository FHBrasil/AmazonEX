 ...
 */
@Component
public class PayPalCheckoutFacade extends DefaultHeringCheckoutFacade {
     ...
     * (non-Javadoc)
     * @see
     * de.hybris.platform.commercefacades.order.impl.DefaultCheckoutFacade#authorizePayment(java
     * .lang.String)
     */
    @Override
    public boolean authorizePayment(final String securityCode) {
        ...
        if (checkIfCurrentUserIsTheCartUser()) {
            ...
            if (infoModel instanceof PaypalPaymentInfoModel) {
                final PaypalPaymentInfoModel paypalModel =
                ...
                if (paypalModel != null && StringUtils.isNotBlank(paypalModel.getToken())) {
                    ...
                    final DoExpressCheckoutPaymentRequestData doExprCheckPaymentReqData =
                            new DoExpressCheckoutPaymentRequestData();
                    doExprCheckPaymentReqData.setToken(paypalModel.getToken());
                    doExprCheckPaymentReqData.setPayerid(paypalModel.getPayerid());
                    doExprCheckPaymentReqData.setPaymentAction(getConfigurationService()
                            .getConfiguration().getString(PaypalConstants.PAYMENT_ACTION));
                    doExprCheckPaymentReqData.setSessionCart(checkoutFacade.getCheckoutCart());
                    // call web service doExpressCheckoutPayment method
                    final DoExpressCheckoutPaymentResultData doExprCheckPaymentResData =
                            payPalPaymentFacade.doExpressCheckoutPayment(doExprCheckPaymentReqData);
                    // update cart with payment info
                    if (PaypalConstants.STATUS_SUCCESS.equals(doExprCheckPaymentResData.getAck())) {
                        final List<PaymentInfoData> paymentInfoList =
                                doExprCheckPaymentResData.getPaymentInfoList();
                        final String paymentAction =
                                StringUtils.capitalize(paypalModel.getPaymentAction().getCode()
                                        .toLowerCase());
                        ...
                        for (final PaymentInfoData paymentInfoData : paymentInfoList) {
                            ...
                            if (PaypalConstants.ORDER_PAYMENT_ACTION_NAME.equals(paymentAction)) {
                                ...
                                if (PaymentStatus.PENDING == paymentInfoData.getPaymentStatus()
                                        && PendingReason.AUTHORIZATION != paymentInfoData
                                                .getPendingReason()) {
                                    orderTransactionEntry =
                                            createTransactionEntry(PaymentTransactionType.ORDER,
                                                    PaymentStatus.PENDING.name(), paymentInfoData
                                                            .getPendingReason().name(),
                                                    transactionId, cartModel, currencyIsoCode,
                                                    amount, paymentDate);
                                } else {
                                    orderTransactionEntry =
                                            createTransactionEntry(PaymentTransactionType.ORDER,
                                ...
                                final DoAuthorizationRequestData doAuthReqData =
                                        new DoAuthorizationRequestData();
                                ...
                                final DoAuthorizationResultData doAuthResData =
                                        payPalPaymentFacade.doAuthorization(doAuthReqData);
                                final Date authDate = doAuthResData.getDateTime().getTime();
                                final String authCurrencyIsoCode =
                                        doAuthResData.getCurrencyIsoCode();
                                final double authAmount = doAuthResData.getAmount();
                                final String authorizationId = doAuthResData.getTransactionId();
                                // result must have pending payment status and pending reason
                                // authorization
                                ...
                                if (PaypalConstants.STATUS_SUCCESS.equals(doAuthResData.getAck())
                                        ...
                                        && PendingReason.AUTHORIZATION == doAuthResData
                                                .getPendingReason()) {
                                    final PaymentTransactionEntryModel authTransactionEntry =
                                            createTransactionEntry(
                                                    ...
                                                    TransactionStatus.ACCEPTED.name(),
                                                    TransactionStatusDetails.SUCCESFULL.name(),
                                                    authorizationId, cartModel, authCurrencyIsoCode,
                                                    authAmount, authDate);
                                    paymentTransactionEntries.add(authTransactionEntry);
                                    // in capture operation authorization id returned by
                                    // doAuthorize method call
                                    // will be used
                                    transactionId = authorizationId;
                                } else if (PaypalConstants.STATUS_SUCCESS.equals(doAuthResData
                                        .getAck())
                                        && PaymentStatus.PENDING == doAuthResData.getPaymentStatus()
                                        && PendingReason.AUTHORIZATION != doAuthResData
                                                .getPendingReason()) {
                                    final PaymentTransactionEntryModel authTransactionEntry =
                                            createTransactionEntry(
                                                    PaymentTransactionType.AUTHORIZATION,
                                                    PaymentStatus.PENDING.name(), doAuthResData
                                                            .getPendingReason().name(),
                                                    authorizationId, cartModel, authCurrencyIsoCode,
                                                    authAmount, authDate);
                                    paymentTransactionEntries.add(authTransactionEntry);
                                    ...
                                    final PaymentTransactionEntryModel failedAuthTransactionEntry =
                                            createTransactionEntry(
                                                    PaymentTransactionType.AUTHORIZATION,
                                                    TransactionStatus.ERROR.name(),
                                                    TransactionStatusDetails.UNKNOWN_CODE.name(),
                                                    authorizationId, cartModel, currencyIsoCode,
                                                    amount, authDate);
                                    paymentTransactionEntries.add(failedAuthTransactionEntry);
                                    return false;
                                }
                            } else if (getConfigurationService().getConfiguration()
                                    .getString(PaypalConstants.PAYMENT_ACTION)
                                    .equals(PaypalConstants.AUTHORIZATION_PAYMENT_ACTION_NAME)) {
                                // in case of AUTHORIZATION payment action, success status of
                                // getExpressCheckoutPayment
                                // operation mean, that authorization of amount also passed
                                // successfully
                                PaymentTransactionEntryModel authTransactionEntry = null;
                                if (PaymentStatus.PENDING == paymentInfoData.getPaymentStatus()
                                        && PendingReason.AUTHORIZATION != paymentInfoData
                                                .getPendingReason()) {
                                    authTransactionEntry =
                                            createTransactionEntry(
                                                    PaymentTransactionType.AUTHORIZATION,
                                                    PaymentStatus.PENDING.name(), paymentInfoData
                                                            .getPendingReason().name(),
                                ...
                                paymentTransactionEntries.add(authTransactionEntry);
                            } else if (PaypalConstants.SALE_PAYMENT_ACTION_NAME
                                    .equalsIgnoreCase(paymentAction)) {
                                PaymentTransactionEntryModel authTransactionEntry = null;
                                if (PaymentStatus.PENDING == paymentInfoData.getPaymentStatus()
                                        && PendingReason.AUTHORIZATION != paymentInfoData
                                                .getPendingReason()) {
                                    authTransactionEntry =
                                            createTransactionEntry(PaymentTransactionType.SALE,
                                                    PaymentStatus.PENDING.name(), paymentInfoData
                                                            .getPendingReason().name(),
                                                    transactionId, cartModel, currencyIsoCode,
                                                    amount, paymentDate);
                            ...
                            final PaymentTransactionModel paymentTransaction =
                                    getModelService().create(PaymentTransactionModel.class);
                        ...
                        return PaypalConstants.STATUS_SUCCESS.equals(doExprCheckPaymentResData
                                .getAck());
                    } else {
     ...
     * @return
     */
    private PaymentTransactionEntryModel createTransactionEntry(final PaymentTransactionType type,
            final String status, final String statusDetails, final String requestId,
            final CartModel cartModel, final String currencyIsoCode, final double amount,
            final Date timeStamp) {
        final PaymentTransactionEntryModel paymentTransactionEntry =
                getModelService().create(PaymentTransactionEntryModel.class);
        ...
        final String code =
                PaypalConstants.PAYMENT_PROVIDER_NAME + "_cart_" + cartModel.getCode() + "_stamp_"
            ...
            while (errorIterator.hasNext()) {
                ...
                if (PaypalConstants.ERROR_CODE_10486.equals(errorCode)) {
                    final UiExperienceLevel uiExperienceLevel =
                            uiExperienceService.getUiExperienceLevel();
                    ...
                    if (StringUtils.isNotEmpty(redirectUrl)) {
                        getSessionService().setAttribute(
                                PaypaladdonConstants.PAY_PAL_REPEAT_REDIRECT_URL, redirectUrl);
                    }
                }
            }
        }
