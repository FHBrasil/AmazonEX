 ...
 */
public class AmazonCancelAction extends ItemAction
{
	private static final Logger LOG = Logger.getLogger(AmazonCancelAction.class);
	...
	public ActionResult perform(final ActionEvent paramActionEvent) throws JaloBusinessException
	{
		...
		try
		{
			credentials.getService().cancelOrderReference(createCancelRequest(credentials, paymentInfo));
			updateInfos(credentials, paymentInfo, (GenericItemChip) paramActionEvent.getSource());
			return new ActionResult(ActionResult.OK, "Order cancelled", false);
		}
	 ...
	 * @return request
	 */
	private CancelOrderReferenceRequest createCancelRequest(final AmazonCredentials credentials, final PaymentInfo data)
	{
