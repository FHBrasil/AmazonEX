...
public class BazaarvoiceProductFeedPerformable extends AbstractJobPerformable<BazaarvoiceProductFeedExportCronJobModel>
{
	...
	public PerformResult perform(final BazaarvoiceProductFeedExportCronJobModel paramT)
	{
		...
		try
		{
			...
			final Map<String, CategoryModel> categoriesMap = new HashMap<String, CategoryModel>();
			final Map<String, CategoryModel> brandsMap = new HashMap<String, CategoryModel>();
			// getting categories from a product catalog
			final List<CategoryModel> superCategories = getRootCategories(catalog);
			setLocales();


			// filling brands array and getting products for categories
			for (final CategoryModel category : superCategories)
			{
				// if a category is a brand - fill brands
				// else fill categories and products
				if (category.getCode().equals("brands"))
				{

					for (final CategoryModel categoryModel : getSubCategories(category))
					{
						brandsMap.put(categoryModel.getCode(), categoryModel);
					}
				}
				else
				{
					...
					for (final CategoryModel categoryModel : getSubCategories(category))
					{
						categoriesMap.put(categoryModel.getCode(), categoryModel);
					}
					for (final ProductModel productModel : productService.getProductsForCategory(category))
					{
			...
			productFeedXmlCreator.setBrands(brands);
			...
			productTags.addAll(productFeedXmlCreator.getProductTagsFromCollection(products, locales, bazaarvoiceConfig
					.getIsFamiliesEnabled().booleanValue()));
			LOG.debug("Count of brands     is " + brands.size());
			LOG.debug("Count of categories is " + categories.size());
			...
			{
				final String fileName = productFeedXmlCreator.generateTodayFilename(xmlBaseFileName, dateNamber);
				final String dir = localXmlOutputDirectory;

				final File dir1 = new File(PROJECT_ROOT_PATH + (dir == null ? "" : dir));
				if (!dir1.exists())
				{
					dir1.mkdirs();
				}
				final File file = new File(dir1.getAbsolutePath(), fileName + ".xml");
				if (!file.exists()) {
					try {
				...
				feed.setBrands(productFeedXmlCreator.getBrandTagsFromCollection(brands, locales));
				feed.setCategories(productFeedXmlCreator.getCategoryTagsFromCollection(categories, locales));
			...
			final List<File> compressedFiles = compressFilesToGz(files, localXmlArchiveDirectory);
	...
	private List<File> compressFilesToGz(final List<File> files, final String archiveDirectory)
	{
		...
		try
		{
			...
			for (final File file : files)
			{
				final File dir = new File(PROJECT_ROOT_PATH + archiveDirectory);
				if (!dir.exists())
				{
					dir.mkdirs();
				}
				outputFile = new File(dir.getAbsolutePath(), file.getName() + ".gz");
	...
	//return false if all parameters not null
	private boolean isEnoughInformation()
	{
		...
		if (localXmlOutputDirectory == null)
		{
			LOG.debug("localXmlOutputDirectory property in the config file is missing.");
			isError = false;
		}
		if (localXmlArchiveDirectory == null)
		{
			LOG.debug("localXmlArchiveDirectory property in the config file is missing.");
			isError = false;
		}
		return isError;
