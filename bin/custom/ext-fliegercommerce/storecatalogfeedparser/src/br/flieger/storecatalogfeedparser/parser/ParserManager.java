...
public class ParserManager {
	...
	private List<String> filter(final Collection<?> src, final String prefix, final EntryPart entryPart) {
		...
		for (Object element: src) {
	        if (element.toString().startsWith(prefix)) {
	        	switch (entryPart) {
					case KEY: result.add(element.toString()); break;
					case VALUE: result.add(properties.getProperty(element.toString()).trim()); break;
				}
	        }
	    }
