...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/">
    <xsl:value-of select="$header" /><xsl:value-of select="$break" />
    <xsl:apply-templates select="products/product" />
  </xsl:template>

  <xsl:template match="product">
  	<xsl:value-of select="code"/><xsl:value-of select="$delim" />
  	<xsl:value-of select="url"/><xsl:value-of select="$break" />
  </xsl:template>
