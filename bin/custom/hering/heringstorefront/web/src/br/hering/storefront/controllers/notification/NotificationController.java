...
//					+ "AND {pte:" + PaymentTransactionEntryModel.ADYENMERCHANTREFERENCE + "} = ?merchantReference ";
//			
...
//			SearchResult<PaymentTransactionEntryModel> result = getFlexibleSearchService().search(query);
...
//	}
	...
	private Notification populateNotificationObject(final ServletRequest request)
	{
		...
		notification.setOriginalReference(request.getParameter(NotificationConstants.ORIGINAL_REFERENCE));
		notification.setMerchantReference(request.getParameter(NotificationConstants.MERCHANT_REFERENCE));
		...
		notification.setMerchantAccountCode(request.getParameter(NotificationConstants.MERCHANTE_ACCOUNT_CODE));
		...
		{
			notification.setSuccess(new Boolean(request.getParameter(NotificationConstants.SUCCESS)));
		}
	...
	private void handleCapture(final Notification notification)
	{
		...
		if (!notification.getPaymentMethod().contains("boletobancario"))
		{
			...
			{
				LOG.info("Order [" + model.getCode() + "] não capturada. Razão [" + notification.getReason() + "]");
		...
		{
			LOG.info("Notificação de CAPTURE para Cartão de Crédito não reconhecida. [" + notification.toString() + "]");
		}
	}
	...
	private void handlePending(final Notification notification)
	{
		...
		if (notification.getNossoNumero() != null)
		{
			...
			LOG.info("Notification PENDING recebida com o Nosso Número [" + notification.getNossoNumero() + "]");
		}
		else
		{
...
//				modelService.save(orderModel);
//				LOG.info("Order [" + orderModel.getCode() + "] com boleto [" + entryModel.getAdyenBoletoNossoNumero()
//						+ "] compensado.");
//			}
//			else
//			{
...
//			}
//			eventService.publishEvent(orderPayedEvent);
			LOG.info("Notificação de pagamento do boleto da Order [" + orderModel.getCode() + "] tratada pelo backend.");
		}
		else
		{
			if (!notification.getSuccess().booleanValue())
			...
			else 
			{
   					...
   					&& orderModel.getPaymentInfo() instanceof CreditCardPaymentInfoModel)
   			{
   				...
   				((CreditCardPaymentInfoModel)orderModel.getPaymentInfo()).setType(creditCardType);
   			}
   			modelService.save(entryModel);
			...
			final PaymentTransactionType transactionType)
	{
		...
		try
		{
			final String fsDefault = 
					...
					+ "FROM {PaymentTransactionEntry AS pte} "
					+ "WHERE {pte:" + PaymentTransactionEntryModel.ADYENREFERENCE + "} = ?adyenReference "
					+ "AND {pte:" + PaymentTransactionEntryModel.ADYENMERCHANTREFERENCE + "} = ?merchantReference "//esta deveria ser o join com a order, mas calma
