 ...
 * 
 * @version 1.1 - Adjustment in the method <code>editPaymentMethod</code> to
 *          create Subscription of Payment.
 * 
 */
@Controller
@Scope("tenant")
...
public class AccountPageController extends AbstractSearchPageController {
	...
	protected HeringAddressForm getPreparedAddressForm() {
		 ...
		 * getDefaultHeringCustomerFacade().getCurrentCustomer
		 * ().getDefaultBillingAddress(); if(defaultAddress != null) {
		 ...
		 * addressForm
		 * .setRegionIso(defaultAddress.getRegion().getIsocodeShort());
		 * addressForm.setLine1(defaultAddress.getLine1());
		 * addressForm.setPhone(
			...
			if (addressData.getId() != null
				...
				model.addAttribute(
						...
						getI18NFacade().getRegionsForCountryIso(
								addressData.getCountry().getIsocode()));
				model.addAttribute("selectedRegion", addressData.getRegion());
				model.addAttribute("country", addressData.getCountry()
						.getIsocode());
				model.addAttribute("addressData", addressData);
				...
				addressForm
						.setCountryIso(addressData.getCountry().getIsocode());
	...
	@RequireHardLogIn
	public String editAddress(final HeringAddressForm addressForm,
			final BindingResult bindingResult, final Model model,
			final RedirectAttributes redirectModel)
			throws CMSItemNotFoundException {
		getAddressValidator().validate(addressForm, bindingResult, model);
	...
	public @ResponseBody String getAddressByZipcode(
		...
		// serviceUrl =
		// "https://www.hering.com.br/smartcheckout/ajax/getAddressByPostcode?zipcode="
		// + zipcode;
