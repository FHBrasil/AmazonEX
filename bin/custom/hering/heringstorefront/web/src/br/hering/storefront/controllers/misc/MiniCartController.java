...
public class MiniCartController extends AbstractController
{
			...
			throws CMSItemNotFoundException
	{
		...
		final List<OrderEntryData> allEntries = cartData.getEntries();
		if (allEntries != null)
		{
			Collections.reverse(allEntries);
			// Implementado conforme descrito em: https://docs.google.com/a/fliegersoftware.de/spreadsheets/d/1usWsm9PpjtIPtFyGPz_Ei10-EKainmBK1pRPs-P-mxc/edit#gid=193042411
			// "No Minicart só deve exibir os produtos daquela loja, não os produtos adicionados em outras lojas (exemplo no e-mail enviado pelo Rubem)"
			List<OrderEntryData> entries = new ArrayList<OrderEntryData>();
			for(OrderEntryData e : allEntries)
				if(e.getProduct().getBrand().equals(baseStoreService.getCurrentBaseStore().getUid()))
					entries.add(e);
