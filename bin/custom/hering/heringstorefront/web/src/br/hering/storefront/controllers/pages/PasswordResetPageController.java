...
public class PasswordResetPageController extends AbstractPageController
{
	...
	{
		final String ER_EMAIL = "[a-z0-9_\\.\\-]*@[a-z0-9\\-_]*\\.[a-z\\.\\-_]*";
		final String ER_CPF_CNPJ = "[0-9]{11}|[0-9]{14}";
		
		String email = null, cpfCnpj = null;
		
		if(form.getEmail().toLowerCase().matches(ER_EMAIL))
		{
			email = form.getEmail();
		}
		else if(form.getEmail().replaceAll("[.]", "").replaceAll("/", "")
		...
		{
			bindingResult.rejectValue("email", "forgottenPwd.invalid");
		}
		
		if (bindingResult.hasErrors())
		{
			GlobalMessages.addErrorMessage(model, "forgottenPwd.invalid");
			return ControllerConstants.Views.Fragments.Password.PasswordResetRequestPopup;
		}
		else
		{			
