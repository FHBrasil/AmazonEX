		...
		<c:choose>
			<c:when test="${consignment.status.code eq 'SHIPPED'}" >
				<strong> Tracking #:</strong> 
					<c:choose>
						<c:when test="${not empty consignment.trackingID}">
							<c:if test="${order.deliveryMode.name == 'TFA'}">
								<a class="" href="http://www.transfolha.com.br/outros/pesquisahttpentrega.asp?scliente=000539&schave=6b7e676029&spesquisa=generico&sdado=${consignment.trackingID}" target="_blank">
									${consignment.trackingID}
								</a>
							</c:if>
							<c:if test="${order.deliveryMode.name == 'CORREIOS'}">
								<a class="" href="http://websro.correios.com.br/sro_bin/txect01$.querylist?p_lingua=001&p_tipo=001&p_cod_uni=${consignment.trackingID}" target="_blank">
									${consignment.trackingID}
								</a>
							</c:if>
							<c:if test="${order.deliveryMode.name == 'JTT'}">
								<a class="" href="http://www.jttlog.com.br/consulta_nf.php?nf=${consignment.trackingID}" target="_blank">
<%-- 									<p>${consignment.trackingID}</p> --%>
								</a>
							</c:if>
