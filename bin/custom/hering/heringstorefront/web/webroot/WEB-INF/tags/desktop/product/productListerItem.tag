...
<c:if test="${themeName == 'black'}">
	...
	<div id="${product.code}" class="productListItem-li">
		<ycommerce:testId code="test_searchPage_wholeProduct">
			...
			<%--<div class="sizes-box">
				<ul>
					<li class="OutOfStock-1">P</li>
					<li class="OutOfStock-1">M</li>
					<li class="InStock-1">G</li>
					<li class="InStock-1">XG</li>
					<li class="InStock-1-Selected">XGG</li>
				</ul>    
			</div>--%>
			...
			<div class="cart-li clearfix">
				...
				<form:form id="addToCartForm${product.code}" action="${addToCartUrl}" 
					...
					<ycommerce:testId code="product_addProduct_button">
						<button type="${buttonType}" class="addToCartButton-li <c:if test="${product.stock.stockLevelStatus.code eq 'outOfStock' }">out-of-stock</c:if>" <c:if test="${product.stock.stockLevelStatus.code eq 'outOfStock' }"> disabled="disabled" aria-disabled="true"</c:if>>${addToCartText}</button>
					</ycommerce:testId>
				</form:form>
			</div>
		</ycommerce:testId>
		...
		<ycommerce:testId code="test_searchPage_wholeProduct">
			...
			<div class="cart-li clearfix">
				...
				<form:form id="addToCartForm${product.code}" action="${addToCartUrl}" 
					...
					<input type="hidden" name="productCodePost" value="${product.code}"/>
					<ycommerce:testId code="product_addProduct_button">
						<button type="${buttonType}" disabled="disabled" class="addToCartButton addToCartButton-li <c:if test="${product.stock.stockLevelStatus.code eq 'outOfStock' }">out-of-stock</c:if>" <c:if test="${product.stock.stockLevelStatus.code eq 'outOfStock' }"> disabled="disabled" aria-disabled="true"</c:if>>${addToCartText}</button>
					</ycommerce:testId>
				</form:form>
			</div>
		</ycommerce:testId>
