...
<%@ attribute name="orderGroup" required="true" type="de.hybris.platform.acceleratorfacades.order.data.OrderEntryGroupData"%>
			...
			<c:forEach items="${orderGroup.entries}" var="entry">
				...
				<tr class="item">
					...
					<td headers="header2" class="details">
						<div class="orderConfirmationProductDetailData">
							<input type="hidden" class="productCode" value="${entry.product.code }" />
							<input type="hidden" class="productPrice" value="${entry.basePrice }" /> 	
