		...
		<c:if test="${hasPreviousPage}">
			...
			<c:forEach begin="${beginPage}" end="${endPage}" var="pageNumber">
				<c:choose>
					...
					<c:otherwise>
						<li id="${( pageNumber -1 )}"><strong>${pageNumber}</strong></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
...
<c:if test="${themeName == 'hering' || themeName == 'dzarm' || themeName == 'foryou'}">
		...
		<c:if test="${hasPreviousPage}">
			<li>
				...
				<ycommerce:testId code="searchResults_previousPage_link">
					<a href="${previousPageUrl}" rel="prev" class="prev"> < <%-- <spring:theme code="${themeMsgKey}.linkPreviousPage"/> --%> </a>
				</ycommerce:testId>
			</li>
		</c:if>
		...
		<c:if test="${hasNextPage}">
			<li>
				...
				<ycommerce:testId code="searchResults_nextPage_link">
					<a href="${nextPageUrl}" rel="next" class="next">  </a>
				</ycommerce:testId>
			</li>
		</c:if>		
	</ul>
