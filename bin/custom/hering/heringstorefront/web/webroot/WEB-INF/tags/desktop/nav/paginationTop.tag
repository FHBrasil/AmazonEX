...
<%@ attribute name="isShowInfo" required="true" type="java.lang.Boolean"%>
<%@ attribute name="isShowListHeringEnabled" required="false" type="java.lang.Boolean"%>
...
<c:if test="${searchPageData.pagination.totalNumberOfResults == 0 && top }">
    ...
    <c:if test="${searchPageData.pagination.totalNumberOfResults > 0}">
                    ...
                    <c:if test="${not isShowListHeringEnabled}">
                        <c:choose>
                            <c:when test="${isShowInfo}">
                                <label class="show-on-desktop"><spring:theme
                                        code="${themeMsgKey}.ocultarInfo" /></label>
                                <a href="#"
                                    onclick="$(this).closest('form').find('input[name=isShowInfo]').val('false');$(this).closest('form').submit()">
                                    <span class="hide-on-desktop"></span> <i class="fa fa-eye"
                                    style="opacity: 0.65;"></i>
                                </a>
                            </c:when>
                            <c:otherwise>
                                <label class="show-on-desktop"><spring:theme
                                        code="${themeMsgKey}.verInfo" /></label>
                                <a href="#"
                                    onclick="$(this).closest('form').find('input[name=isShowInfo]').val('true');$(this).closest('form').submit()">
                                    <span class="hide-on-desktop"></span> <i
                                    class="fa fa-eye selected"></i>
                                </a>
                            </c:otherwise>
                        </c:choose>
                    </c:if>
        ...
        <div class="diagramming">
            <form action="#" method="GET">
                <label><spring:theme code="${themeMsgKey}.diagramacao" /></label> <input
                    type="hidden" name="q" value="${searchPageData.currentQuery.query.value}">
                <input type="hidden" name="isShowListHeringEnabled" id="isShowListHeringEnabled">
                <c:choose>
                    <c:when test="${isShowListHeringEnabled || isShowListEnabled}">
                        <a class="fa fa-th-large" style="cursor: pointer;"
                            onclick="$(this).closest('form').find('input[name=isShowListHeringEnabled]').val('false');$(this).closest('form').submit()"></a>
                        <a class="fa fa-list selected"></a>
                    </c:when>
                    <c:otherwise>
                        <a class="fa fa-th-large selected"></a>
                        <a class="fa fa-list" style="cursor: pointer;"
                            onclick="$(this).closest('form').find('input[name=isShowListHeringEnabled]').val('true');$(this).closest('form').submit()"></a>
                    </c:otherwise>
                </c:choose>
            </form>
        </div>
        ...
        <div class="relevance">
            <c:if test="${not empty searchPageData.sorts}">
                <form id="sort_form${top ? '1' : '2'}" name="sort_form${top ? '1' : '2'}"
                    method="get" action="#">
                    <input type="hidden" name="isShowInfo" id="isShowInfo"> <select
