...
<table id="orderTotals">
	...
	<tbody>
		...
		<c:if test="${not hasAppliedValeCredito and cartData.totalDiscounts.value > 0}">
			<tr class="savings">
		...
		<c:if test="${hasAppliedValeCredito and cartData.totalDiscounts.value > 0}">
			<tr class="savings">
					<td><spring:theme code="basket.page.totals.savings" /></td>
					<td><ycommerce:testId code="Order_Totals_Savings">
							<format:price priceData="${totalDiscounts}" />
						</ycommerce:testId></td>
				</tr>		
		</c:if>
		...
		<c:if test="${hasAppliedValeCredito}">
			<tr class="savings">
				<td><spring:theme code="basket.page.totals.valecredito" /></td>
				<td><ycommerce:testId code="Order_Totals_Savings">
						<format:price priceData="${valeCreditoValue}" />
					</ycommerce:testId></td>
			</tr>
		</c:if>
		...
		<c:if test="${not empty cartData.deliveryCost}">
			<tr>
				...
				<td><format:price priceData="${cartData.deliveryCost}"
						displayFreeForZero="TRUE" /> <c:if
						test="${not empty cartData.deliveryMode}">
						<br>
