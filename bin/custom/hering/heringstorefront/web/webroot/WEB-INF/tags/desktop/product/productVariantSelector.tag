...
<c:choose>
    ...
    <c:otherwise>
        ...
        <c:if
            test="${(not empty product.baseOptions[0].options) and (product.baseOptions[0].variantType eq 'HeringStyleVariantProduct')}">
        ...
        <c:if
            test="${(not empty product.baseOptions[1].options) and (product.baseOptions[0].variantType eq 'HeringSizeVariantProduct')}">
        ...
        <c:forEach items="${product.baseOptions}" var="baseOptions">
            <c:forEach items="${baseOptions.selected.variantOptionQualifiers}"
                ...
                <div class="cores ${product.stock.stockLevel <= 0 ? 'semEstoque' : ''}">
                    ...
                    <ul class="bxslider-cores">
                        <c:forEach items="${variantStyles}" var="variantStyle" varStatus="status">
                            ...
                            <li data-variantcode="${variantStyle.code}"
                                color="${variantStyle.color.RGB}"
                                class="${styleValue} variantSelectorHandler<c:if test="${variantStyle.url eq currentStyleUrl}"> active</c:if>"
                                style="background-color:${variantStyle.color.RGB};"><input
                                type="hidden" name="codeData"
                                value="${variantStyle.code}-${variantStyle.code.substring(0,4)}${variantStyle.color.RGB}">
                                <a href="${productStyleUrl}"
                                <c:if test="${selectedColor eq styleValue}">class="active"</c:if>></a>
                            </li>
                        </c:forEach>
                    </ul>
                </div>
            </c:if>
            <c:set var="classSemEstoque"
                value="${product.stock.stockLevel <= 0 ? 'semEstoque semEstoqueSizes' : ''}" />
            <c:set var="classDiv70SemEstoque"
                value="${fn:length(product.sizes) >= 2 ? 'classDiv70SemEstoque' : ''} " />
                    ...
                    <c:when test="${useSizesProperty}">
                        <ul>
                            <c:forEach items="${product.sizes}" var="size" varStatus="status">
                                ...
                                <li data-variantcode="${variantSize.code}"
                                    class="variantSelectorHandler"><a
                                    href="${variantOptionUrl}"
                                    <c:if test="${selectedSize eq  sizesString}">class="active"</c:if>>${sizesString}</a></li>
                            </c:forEach>
                        </ul>
                    </c:when>
                    <c:otherwise>
                        ...
                        <c:if test="${not empty variantSizes}">
                                ...
                                <c:forEach items="${variantSizes}" var="variantSize">
                                    ...
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            ...
                                            <li data-variantcode="${variantSize.code}"
                                                class="variantSelectorHandler"><a
                                                href="${variantOptionUrl}" class="out-of-stock">${sizesString}</a></li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                            </ul>
                            <c:if test="${selectedSize eq  sizesString}">
                                <div class="chaordic-size">
                                    <input type="hidden" class="chaordic-size-selected"
                                        value="${sizesString}" />
                                </div>
                            </c:if>
                            <c:if test="${showVariantSizeGuide and not empty product.measurements}">
                                <a
                                    class="fancybox tabela-medidas ${product.stock.stockLevel <= 0 ? 'aSemEstoque' : ''}"
                                    href="#modal-tabela-medidas"> <spring:theme
