		...
		<ycommerce:testId code="facetNav_facet${facetData.name}_links">
			<div class="facetValues">
				<c:if test="${not empty facetData.topValues}">
					...
					<div class="topFacetValues ${facetData.name eq 'cor' ? 'cores' : ''} ">
								...
								<li>
									<c:if test="${facetData.multiSelect}">
										<form action="#" method="get">
											...
											<label class="facet_block-label tipo${facetData.name}" style="${colorStyle}">
												<input class="InStock-class" type="checkbox" ${facetValue.selected ? 'checked="checked"' : ''} onchange="$(this).closest('form').submit()"/>
												${facetValue.name}&nbsp;
				...
				<div class="allFacetValues ${facetData.name eq 'cor' ? 'cores' : ''} ${fn:containsIgnoreCase(facetData.name, 'Tamanho') ? 'tamanhos' : ''}" style="${not empty facetData.topValues ? 'display:none' : ''}">
					<ul class="facet_block ${facetData.multiSelect ? '' : 'indent'}">
	...
	<c:if test="${not empty facetData.values}">
		...
		<ycommerce:testId code="facetNav_facet${facetData.name}_links">
				...
				<c:if test="${not empty facetData.topValues}" >
						...
						<ul class="topFacetValues">
							<c:forEach items="${facetData.topValues}" var="facetValue">
									...
									<c:if test="${facetData.multiSelect}">
										...
										<li style="${themeName == 'foryou' ? colorStyle : ''}">
											<form action="#" method="get">
												<input type="hidden" name="q" value="${facetValue.query.query.value}"/>
												
												
												<c:choose>
												    <c:when test="${not fn:containsIgnoreCase(facetData.name, 'subcategoria')}">
												      <label class="facet_block-label tipo${facetData.name}" style="${colorStyle}" onclick="$(this).closest('form').submit()">
															<input class="InStock-class" type="checkbox" ${facetValue.selected ? 'checked="checked"' : ''} />
															${facetValue.name}												
														</label>
												    </c:when>
												</c:choose>
						...
						<c:if test="${not empty facetData.topValues}">
							<c:if test="${not  fn:containsIgnoreCase(facetData.name, 'subcategoria')}">							
								<c:if test="${themeName == 'foryou'}">
									<a href="javascript:void(0);" class="show-all"><spring:theme code="search.nav.facetShowMore_${facetData.code}" /></a>
								</c:if>
								<c:if test="${themeName != 'foryou'}">
									<a href="#" class="show-all"><spring:theme code="search.nav.facetShowMore_${facetData.code}" /></a>
								</c:if>
							</c:if>
						</c:if> 
		...
		<c:if test="${not empty facetData.topValues}" >
			<c:choose>
				...
				<c:otherwise>
					<c:if test="${themeName == 'foryou'}">
						<c:set var="idUl" value="${fn:containsIgnoreCase(facetData.name, 'cor') ? 'ulMaisCores' : 'ulMaisCategorias'}" />
						<ul id="${idUl}" style="${fn:containsIgnoreCase(facetData.name, 'cor') ? 'display: none;' : ''}  ${fn:containsIgnoreCase(facetData.name, 'categoria') ? 'display: none;' : ''}  ">
					</c:if>
					<c:if test="${themeName != 'foryou'}">
						<ul style="${fn:containsIgnoreCase(facetData.name, 'cor') ? 'display: none;' : ''}  ${fn:containsIgnoreCase(facetData.name, 'categoria') ? 'display: none;' : ''}  ">
					</c:if>
				</c:otherwise>
			</c:choose>		
		</c:if>	
		...
		<c:if test="${fn:containsIgnoreCase(facetData.name, 'subcategoria')}">
			<input type="search" placeholder="Buscar" class="search-box">
				...
				<c:if test="${facetData.name eq 'cor'}">
					...
					<li class="${facetData.name eq 'cor' ? 'cores' : ''}" style="${themeName == 'foryou' and facetData.name eq 'cor' ? colorStyle : ''}">		
						<c:if test="${facetData.multiSelect}">
							<form action="#" method="get">
								<input type="hidden" name="q" value="${facetValue.query.query.value}"/>
								<c:choose>
								    ...
								    <c:when test="${fn:containsIgnoreCase(facetData.name, 'Tamanho')}">
                                 <c:if test="${!facetValue.selected}">
                                      <a onclick="$(this).closest('form').submit()"> ${facetValue.name} </a>
                                 </c:if>
                            </c:when>
					...
					<c:if test="${fn:containsIgnoreCase(facetData.name, 'Tamanho')}">
						<a class="fancybox tabela-medidas guide" href="#modal-tabela-medidas"><spring:theme code="product.variants.size.guide"/></a>
