				...
				<c:choose>
					...
					<c:otherwise>
						<c:if test="${product.oldPrice > product.price.value}">
							<!-- Sale Enable -->
							<li class="promocao">
								<spring:theme code="product.attractions.sale.dzarm" />
							</li>
						</c:if>
					</c:otherwise>
				</c:choose>
...
<%-- 			<form:form id="addToCartForm${product.code}" action="${addToCartUrl}" method="fpost" class="addToCartForm clear_fix"> --%>
<%-- 				<input type="hidden" id="productCodePost" name="productCodePost" value="${product.code}" /> --%>
				<a class="btn btn-cta add addToCartButton" href="${viewMoreUrl}" style="display: block;">Veja mais detalhes</a>
<%-- 			</form:form> --%>
