	...
	<nav id="top-navigation">
		<div class="container">
			...
			<c:if test="${pageType != 'SINGLESTEPCHECKOUT'}">
				<ul class="utils">
					<li class="radio"></li>
					<li class="cadastre-se logged-in">
					...
					<!-- CODIGO ADAPTADO DA DZARM ABAIXO -->
					<sec:authorize ifNotGranted="ROLE_ANONYMOUS">
						<c:set var="maxNumberChars" value="25"/>
						<c:if test="${fn:length(user.firstName) gt maxNumberChars}">
							<c:set target="${user}" property="firstName" value="${fn:substring(user.firstName, 0, maxNumberChars)}..."/>
						...
						<div class="login-box">
							<ycommerce:testId code="header_LoggedUser">						 
								<div>
	            					...
	            					<a href="${encodedUrl}" class="btn"><i class="fa fa-th-list"></i><spring:theme code="header.account.orders" /></a>
	            					<c:url value="/my-account" var="encodedUrl"/>
	            					<a href="${encodedUrl}" class="btn"><i class="fa fa-user"></i><spring:theme code="header.link.account" /></a>
	            				</div>
								<div class="acc-links">
	            					<%-- <a href="#"><i class="fa fa-angle-right"></i> Minhas trocas e devoluções</a> --%>	
									<c:url value="/my-account/profile" var="encodedUrlAddress" />
	            					<a href="${encodedUrlAddress}"><i class="fa fa-angle-right"></i> <spring:theme code="header.account.profile" /></a>
	            					<c:url value="/my-account/address-book" var="encodedUrlAddress" />
	            					<a href="${encodedUrlAddress}"><i class="fa fa-angle-right"></i> <spring:theme code="header.account.addressbook" /></a>
	            					<sec:authorize ifNotGranted="ROLE_ANONYMOUS">
	            						<ycommerce:testId code="header_signOut">
	            							<a href="<c:url value='/logout'/>"><i class="fa fa-angle-right"></i>
						...
						<div class="login-box">
							<ycommerce:testId code="header_LoggedUser">
								...
								<div class="acc-links">
									...
									<a href="<c:url value="/login?forgotPassword=true"/>"><i class="fa fa-angle-right"></i> esqueci minha senha</a>
								</div>
							</ycommerce:testId>
						</div>
					</sec:authorize>
						
					<!-- CODIGO ORIGINAL DA HERING ABAIXO -->
					<%-- 	<div class="login-box">
	           				<div>
