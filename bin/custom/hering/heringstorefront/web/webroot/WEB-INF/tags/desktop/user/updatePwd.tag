...
<c:if test="${themeName == 'hering' || themeName == 'dzarm' || themeName == 'foryou'}">
	<section class="account-edit-password page with-sidebar">
		<section id="acc-password-form">
			...
			<form:form method="post" commandName="updatePwdForm">							
				<div class="form-row"><formElement:formPasswordBox idKey="updatePwd-pwd" labelKey="updatePwd.pwd" path="pwd" inputCSS="text password strength" mandatory="true"/></div>
				<div class="form-row"><formElement:formPasswordBox idKey="updatePwd.checkPwd" labelKey="updatePwd.checkPwd" path="checkPwd" inputCSS="text password" mandatory="true" errorPath="updatePwdForm"/></div>
				<p class="required-info"><spring:theme code="form.required" text="Fields marked * are required"/></p>
				<div class="btn-group">
				<ycommerce:testId code="update_update_button">
					<button class="btn btn-confirmar"><spring:theme code="updatePwd.submit"/></button>
				</ycommerce:testId>
				</div>
			</form:form>
		</section>		
