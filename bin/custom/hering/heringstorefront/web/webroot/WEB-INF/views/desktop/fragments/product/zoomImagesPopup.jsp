...
<div class="productImage">
	<div class="productImageGallery" style="display:none">
			...
			<div class="scroller">
					...
					<ul class="jcarousel-skin">
						<c:forEach items="${galleryImages}" var="container">
							<li>
								<span class="thumb">
										<img src="${container.thumbnail.url}" data-zoomurl="${container.superZoom.url}" alt="${product.name}" title="${product.name}">
								</span>
							</li>
						</c:forEach>
					</ul>
	...
	<c:if test="${empty zoomImageUrl}">
		<c:set value="${ycommerce:productImage(product, 'superZoom').url}" var="zoomImageUrl"/>
	</c:if>
	<c:if test="${not empty requestParams['mediaUrl'][0]}">
		<c:forEach items="${galleryImages}" var="container">
			<c:if test="${container.product.url eq requestParams['mediaUrl'][0]}">
				<c:set var="zoomImageUrl">${container.superZoom.url}</c:set>
			</c:if>
		</c:forEach>
	</c:if>
