											...
											<c:choose>
												...
												<c:otherwise>
													<div class="span-6 productDetailsHolder ${(status.index+1)%3 == 0 ? ' last' : ''}${(status.index)%3 == 0 ? ' first clear' : ''}">
														...
														<c:choose>
															<c:when test="${themeName == 'hering'}"><product:productListerGridItem product="${product}"/></c:when>
															<c:otherwise><product:productListerGridItemDzarm product="${product}"/></c:otherwise>
														</c:choose>
													</div>
												</c:otherwise>
											</c:choose>
