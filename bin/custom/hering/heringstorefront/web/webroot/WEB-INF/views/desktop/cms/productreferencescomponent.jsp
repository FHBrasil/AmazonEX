...
<c:choose>
	<c:when test="${not empty productReferences and component.maximumNumberProducts > 0}">
		...
		<c:if test="${themeName == 'hering' || themeName == 'dzarm' || themeName == 'foryou'}">
			<style>.caracteristicas{width:50%!important}</style>
			<c:set var="referenceType" value="${component.productReferenceTypes[0]}"/>
			<c:choose>
				<c:when test="${referenceType == 'CROSSELLING'}">
					<header>
						<h1>Compre tamb&eacute;m estes produtos e <b>tenha um look completo</b></h1>
					</header>
				</c:when>
				<c:otherwise>
					<div class="container">
						<section class="product-section">
							<header>
								...
								<a href="#" class="view-more"><spring:theme code="storeFinder.see.more"/></a>
							</header>
						</section>
					</div>
				</c:otherwise>
		...
		<c:if test="${referenceType == 'OTHERS'}">
			...
			<div class="section-mini-content ${positionClass}">
				...
				<c:forEach items="${productReferences}" var="productReference">
					<div class="product productDetailsHolder product-mini with-checkbox" id="${productReference.target.code.substring(0,4)}">
						<input type="checkbox" name="crossProduct" value="${productReference.target.code}" />
						<product:productPrimaryImage product="${productReference.target}" format="product"/>
						<div class="info">
							<c:url var="productReferenceUrl" value="${productReference.target.url}"/>
							<h2><a href="${productReferenceUrl}">${productReference.target.name}</a></h2>
							<c:if test="${productReference.target.freeShipping || productReference.target.newProduct || productReference.target.oldPrice > productReference.target.price.value}">
								<div class="selos">
									<ul>
										<c:if test="${productReference.target.oldPrice > productReference.target.price.value}">
								...
								<c:choose>
									<c:when test="${productReference.target.fromPrice}">
										...
										<s>
											...
											<fmt:formatNumber value="${productReference.target.oldPrice}" type="number" minFractionDigits="2" />
										</s>
										<strong>
								...
								<p>
									<c:choose>
										<c:when test="${productReference.target.priceParcels > 1}">
											...
											<b>
												<spring:theme code="product.currencynotation"/>
												<fmt:formatNumber value="${productReference.target.parcelUnitPrice}" type="number" minFractionDigits="2" />
											</b> 
										</c:when>
										<c:otherwise>
							...
							<c:choose>
							<c:when test="${productReference.target.purchasable && productReference.target.stock.stockLevelStatus.code != 'outOfStock'}">
								<product:productVariantSelector product="${productReference.target}" useSizesProperty="false"/>
							</c:when>
							<c:otherwise>
							<div class="btn-group">
				...
				<div class="comprar-junto hide">
					<div class="left">
						<div>Valor dos <b id="quantity"></b> juntos</div>
						<div>fica em <b id="totalPrice"></b></div>
