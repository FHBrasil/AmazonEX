...
(function($) {
    $.fn.Thumbelina = function(settings) {
        ...
        listDimension = 0, // Size (width or height depending on orientation)
                            // of list element.
        idle = 0, outerFunc, orientData; // Stores function calls and CSS
                                            // attribute for horiz or vert mode.
        
        // Add thumblina CSS class, and create an inner wrapping container,
        // within which the list will slide with overflow hidden.
        $list
                .addClass('thumbelina')
                .wrap(
        ...
        // Depending on vertical or horizontal, get functions to call and CSS
        // attribute to change.
        ...
        var animate = function() {
            ...
            // If no movement or resize for 100 cycles, then go into 'idle' mode
            // to save CPU.
            if (!moveDir && pos === destPos
            ...
            // This will also cause the thumbs to drag back out again when
            ...
            if (destPos === minPos)
                settings.$fwdBut.addClass('disabled');
            else
                settings.$fwdBut.removeClass('disabled');
            if (destPos === 0)
                settings.$bwdBut.addClass('disabled');
            else
                settings.$bwdBut.removeClass('disabled');
            
            // Animate towards destination with a simple easing calculation.
            pos += (destPos - pos) / settings.easing;
            
            // If within 1000th of a pixel to dest, then just 'snap' to exact
            // value.
            // Do this so pos will end up exactly == destPos (deals with
            // rounding errors).
            if (Math.abs(destPos - pos) < 0.001)
                pos = destPos;
    ...
    $.fn.Thumbelina.defaults = {
        ...
        $fwdBut : null
    // jQuery element used as forward button.
    };
