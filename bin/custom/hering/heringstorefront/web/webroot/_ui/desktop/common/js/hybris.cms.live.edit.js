ACC.liveEdit = {
    ...
    bindGlobalClick : function() {
        ...
        $('body')
                .click(
                        function(event) {
                            var cmsComponent = ACC.liveEdit
                                    .findNearestCMSComponent(event);
                            if (cmsComponent.length > 0) {
                                var cmsComponentUid = cmsComponent
                                        .data('cmsComponent');
                                var cmsContentSlotUid = cmsComponent
                                        .data('cmsContentSlot');
                                
                                ACC.liveEdit.displayCMSComponentEditor(
                                        cmsComponentUid, cmsContentSlotUid);
                                return false;
                            }
                        });
    },
    ...
    displayCMSComponentEditor : function(cmsComponentUid, cmsContentSlotUid) {
        if (undefined != cmsComponentUid && cmsComponentUid != ""
                && parent.notifyIframeZkComponent) {
