 ...
 * @version 1.1 - Removed clause <code>checkIfCurrentUserIsTheCartUser()</code> from method
 *          <code>createPaymentSubscription</code>.
 */
public class DefaultHeringCheckoutFacade extends DefaultAcceleratorCheckoutFacade implements
        HeringCheckoutFacade {
     ...
     */
    public void setBoletoPaymentInfoReversePopulator(
            BoletoPaymentInfoReversePopulator boletoPaymentInfoReversePopulator) {
     ...
     * (non-Javadoc)
     * @see
     * br.hering.facades.facades.order.HeringCheckoutFacade#setCustomPaymentInfo(com.flieger.payment
     * .data.
    ...
    public void setCustomPaymentInfo(CartData cartData) {
        ...
        if (cartData.getCustomPaymentInfo() instanceof BoletoPaymentInfoData) {
            ...
            final BoletoPaymentInfoModel boletoPaymentInfoModel =
                    getModelService().create(BoletoPaymentInfoModel.class);
            boletoPaymentInfoModel.setUser(getCurrentUserForCheckout());
            boletoPaymentInfoModel.setCode(getCurrentUserForCheckout().getUid() + "_"
                    + UUID.randomUUID());
            boletoPaymentInfoModel.setBillingAddress(addressModel);
            getBoletoPaymentInfoReversePopulator().populate(
                    (BoletoPaymentInfoData) cartData.getCustomPaymentInfo(), boletoPaymentInfoModel);
            ...
            final AddressModel addressModel = getModelService().create(AddressModel.class);
            final AdvancePaymentInfoModel paymentInfoModel =
                    getModelService().create(AdvancePaymentInfoModel.class);
            ...
            getAdvancePaymentInfoReversePopulator().populate(
                    (AdvancePaymentInfoData) cartData.getCustomPaymentInfo(), paymentInfoModel);
    ...
    private void saveVoucherPaymentInfoIntoCart(CartData cartData) {
        final CartModel cart = getCart();
        final VoucherPaymentInfoModel voucherPaymentInfoModel =
                getModelService().create(VoucherPaymentInfoModel.class);
        voucherPaymentInfoModel.setUser(getCurrentUserForCheckout());
        voucherPaymentInfoModel.setCode(getCurrentUserForCheckout().getUid() + "_"
                + UUID.randomUUID());
    ...
    private void saveDebitPaymentInfoIntoCart(final HeringDebitPaymentInfoData debitPaymentInfoData) {
        ...
        HeringDebitPaymentInfoModel debitPaymentInfoModel =
                getModelService().create(HeringDebitPaymentInfoModel.class);
    ...
    // getCustomerAccountService().getPaymentInfoForCode(
     ...
     * (non-Javadoc)
     * @see
     * de.hybris.platform.commercefacades.order.impl.DefaultCheckoutFacade#createPaymentSubscription
     * (de.hybris.platform
    ...
    public CCPaymentInfoData createPaymentSubscription(final CCPaymentInfoData paymentInfoData) {
        ...
        // dois parametros que não são salvos no BD, mas tem que ser mantido na sessão até o user
        // finalizar a compra
        ...
        final CardType cardType =
                getCommerceCardTypeService().getCardTypeForCode(paymentInfoData.getCardType());
        final CreditCardType creditCardType =
                (cardType == null ? CreditCardType.VISA : cardType.getCode());
        ...
        final CreditCardPaymentInfoModel ccPaymentInfoModel =
                getCustomerAccountService().createPaymentSubscription(customerModel, cardInfo,
                        billingInfo, billingAddressData.getTitleCode(), getPaymentProvider(),
    ...
    public List<? extends DeliveryModeData> getSupportedDeliveryModes() {
        ...
        // {
        // final List<DeliveryModeData> result =
        // carrierDeliveryCalculationFacade.getSupportedDeliveryModes(getCart());
        //
        ...
        // {
        // LOG.debug(r.getCode() + " - " + r.getDeliveryCost().getFormattedValue() + " - " +
        // r.getEstimatedDeliveryDays());
        // }
        //
        // return result;
        // }
    ...
    private void prepareCheaper(List<DeliveryModeData> result) {
        ...
        if (threshold != null && getCart().getSubtotal().doubleValue() >= threshold.doubleValue()) {
            final PriceData freeValue =
                    getPriceDataFactory().create(PriceDataType.BUY, BigDecimal.valueOf(0),
                            cheaper.getDeliveryCost().getCurrencyIso());
    ...
    protected DeliveryModeData convert(final DeliveryModeModel deliveryModeModel) {
        ...
        // }
        //
        // return carrierDeliveryCalculationFacade.convert((CarrierZoneDeliveryModeModel)
        // deliveryModeModel, getCart());
    }
     ...
     * (non-Javadoc)
     * @see
     * de.hybris.platform.commercefacades.order.impl.DefaultCheckoutFacade#containsTaxValues()
     */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * br.hering.facades.facades.order.HeringCheckoutFacade#setPaymentMode(br.hering.facades.order
     * .data.PaymentModeData)
     */
    @Override
    public void setPaymentMode(PaymentModeData paymentModeData) {
        final CartModel cartModel = getCart();
        final PaymentModeModel paymentModeModel =
                paymentModeService.getPaymentModeForCode(paymentModeData.getCode());
    ...
    public boolean authorizeBoleto() {
        ...
        if (checkIfCurrentUserIsTheCartUser()) {
            final PaymentTransactionEntryModel paymentTransactionEntryModel =
                    getHeringCheckoutService().authorizePayment(cartModel, null,
                            getPaymentProvider());
            return paymentTransactionEntryModel != null
                    && (TransactionStatus.ACCEPTED.name().equals(
                            paymentTransactionEntryModel.getTransactionStatus()) || TransactionStatus.REVIEW
    ...
    /**
     * @param voucherPaymentInfoReversePopulator
     *            the voucherPaymentInfoReversePopulator to set
     */
    public void setVoucherPaymentInfoReversePopulator(
            VoucherPaymentInfoReversePopulator voucherPaymentInfoReversePopulator) {
    ...
    public void setDebitPaymentInfoReversePopulator(
            DebitPaymentInfoReversePopulator debitPaymentInfoReversePopulator) {
    ...
    private PaymentModeData getPaymentMode() {
        ...
        if (cart != null) {
            ...
            if (paymentModeModel != null) {
                new PaymentModePopulator().populate(paymentModeModel, paymentModeData);
    ...
    protected VoucherPaymentInfoData getVoucherPaymentDetails() {
        ...
        if (cart != null) {
            ...
            if (paymentInfo instanceof VoucherPaymentInfoModel) {
                // getVoucherPaymentInfoReversePopulator().getVoucherPaymentInfoConverter().convert(
                // (VoucherPaymentInfoModel) paymentInfo);
                return getVoucherPaymentInfoPopulator().getVoucherPaymentInfoConverter().convert(
                        (VoucherPaymentInfoModel) paymentInfo);
                // return
                // getVoucherPaymentInfoReversePopulator().getVoucherPaymentInfoConverter().convert((VoucherPaymentInfoModel)
                // paymentInfo);
                // return getVoucherPaymentInfoConverter().convert( (VoucherPaymentInfoModel)
                // paymentInfo);
                // return getCreditCardPaymentInfoConverter().convert((CreditCardPaymentInfoModel)
                // paymentInfo);
            }
        }
        return null;
        ...
        if (cartModel != null) {
            ...
            if (paymentInfoModel instanceof HeringDebitPaymentInfoModel) {
                return debitPaymentInfoConverter
                        .convert((HeringDebitPaymentInfoModel) paymentInfoModel);
                ...
                getBoletoPaymentInfoPopulator().populate((BoletoPaymentInfoModel) paymentInfoModel,
                        boletoPaymentInfoData);
                ...
                getVoucherPaymentInfoPopulator().populate(
                        (VoucherPaymentInfoModel) paymentInfoModel, voucherPaymentInfoData);
                ...
                getAdvancePaymentInfoPopulator().populate(
                        (AdvancePaymentInfoModel) paymentInfoModel, data);
                return data;
            }
        }
        return null;
     ...
     * (non-Javadoc)
     * @see
     * br.hering.facades.facades.order.HeringCheckoutFacade#formatInstallmentsCost(de.hybris.platform
    ...
    /**
     * @param voucherPaymentInfoPopulator
     *            the voucherPaymentInfoPopulator to set
     */
    public void setVoucherPaymentInfoPopulator(
            VoucherPaymentInfoPopulator voucherPaymentInfoPopulator) {
     ...
     */
    @Override
    public HeringDebitPaymentInfoData createPaymentSubscription(
            final HeringDebitPaymentInfoData heringDebitPaymentInfoData) {
        validateParameterNotNullStandardMessage("heringDebitPaymentInfoData",
                heringDebitPaymentInfoData);
        ...
        if (checkIfCurrentUserIsTheCartUser()) {
            ...
            billingInfo.setAddressType(billingAddressData.getType().toString());
            final CustomerModel customerModel = getCurrentUserForCheckout();
            HeringDebitPaymentInfoModel model =
                    getModelService().create(HeringDebitPaymentInfoModel.class);
            ...
            final HeringDebitPaymentInfoModel resultModel =
                    getCustomerAccountService().createPaymentSubscription(customerModel, model,
                            billingInfo, billingAddressData.getTitleCode(), getPaymentProvider(),
                            heringDebitPaymentInfoData.isSaved());
            if (resultModel != null) {
    ...
    public boolean authorizeDebitPayment() {
        ...
        final HeringDebitPaymentInfoModel debitPaymentInfoModel =
                (HeringDebitPaymentInfoModel) cartModel.getPaymentInfo();
        if (checkIfCurrentUserIsTheCartUser()) {
            if (debitPaymentInfoModel != null
                    && StringUtils.isNotBlank(debitPaymentInfoModel.getSubscriptionId())) {
                final PaymentTransactionEntryModel paymentTransactionEntryModel =
                        getHeringCheckoutService().authorizeDebitPayment(cartModel,
                                getPaymentProvider());
                return paymentTransactionEntryModel != null
                        && (TransactionStatus.ACCEPTED.name().equals(
                                paymentTransactionEntryModel.getTransactionStatus()) || TransactionStatus.REVIEW
    ...
    public boolean authorizeDebitPayment3D(final String paymentInfoPK) {
        PaymentInfoModel paymentInfoModel =
                getCustomerAccountService().getPaymentInfoForCode(getCurrentUserForCheckout(),
                        paymentInfoPK);
        if (paymentInfoModel != null && paymentInfoModel instanceof HeringDebitPaymentInfoModel) {
            if (checkIfCurrentUserIsTheCartUser()) {
     ...
     * (non-Javadoc)
     * @see
     * de.hybris.platform.commercefacades.order.impl.DefaultCheckoutFacade#getCurrentUserForCheckout
     * ()
     */
    @Override
    ...
    /*
     * (non-Javadoc)
     * @see br.hering.facades.facades.order.HeringCheckoutFacade#removeAddress(de.hybris.platform.
     * commercefacades.user.data.AddressData)
     */
    @Override
