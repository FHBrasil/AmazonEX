 ...
 *  
 */
package br.hering.cockpits.setup;
...
import br.hering.cockpits.constants.HeringCockpitsConstants;
 ...
 */
@SystemSetup(extension = HeringCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(HeringCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(HeringCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						HeringCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				importImpexFile(context, "/heringcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
			}
