ACC.payment = {
	...
	{
		$('input[name="paymentMode"]:radio').each(function()
		{
			if($(this).attr('checked'))
			{
				if ($(this).val() == "CreditCard") {
					$("#cardDetails").show();
					$("#cardDetails").height("580");
					
					$("#debitDetails").hide();
					$("#debitDetails").height("auto");
				} else if($(this).val() == "Debit") {
					$("#cardDetails").hide();
					$("#cardDetails").height("auto");
					
					$("#debitDetails").show();
					$("#debitDetails").height("100");
				} else {
					$("#cardDetails").hide();
					$("#cardDetails").height("auto");
					$("#debitDetails").hide();
...
{
	ACC.payment.changePaymentMode();
	
	if ($('input[name="paymentMode"]:radio').length > 0)
	{
		$('input[name="paymentMode"]:radio').change( function()
		{
			ACC.payment.changePaymentMode();
		});
	}
