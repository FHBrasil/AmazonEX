 ...
 */
public class DzarmSizeProductReferenceTargetStrategy implements ProductReferenceTargetStrategy
{
	...
	{
		if (sourceProduct instanceof HeringSizeVariantProductModel
				&& reference.getTarget() instanceof HeringStyleVariantProductModel)
		{
			...
			if (CollectionUtils.containsAny(sourceSuperCategories, targetSuperCategories))
			{
				...
				final String size = ((HeringSizeVariantProductModel) sourceProduct).getSize();
				for (final VariantProductModel variant : reference.getTarget().getVariants())
				{
					if (variant instanceof HeringSizeVariantProductModel
							&& size.equals(((HeringSizeVariantProductModel) variant).getSize()))
					{
						return variant;
					}
				}
