...
{	
	...
	protected void onSiteEvent(final OrderCompletedEvent orderCompletedEvent)
	{
		...
		ServicesUtil.validateParameterNotNullStandardMessage("payment info", orderModel.getPaymentInfo());
		...
		if(orderModel.getPaymentInfo() instanceof BoletoPaymentInfoModel) {
		...
		} else if (orderModel.getPaymentInfo() instanceof CreditCardPaymentInfoModel){//creditcard is captured by Adyen
   		...
   		{
   			LOG.info(String.format("order:{%s}; fraudReport:{statusCode:{%s}. Is ready for capture", 
   					orderModel.getCode(), 
   			...
   			LOG.info(String.format("order:{code:%s}; fraudReport:{statusCode:%s}. Is NOT ready for capture",
   					orderModel.getCode(),
	...
	protected boolean shouldHandleEvent(final OrderCompletedEvent event)
	{
		...
		LOG.info(String.format("Order: [%s] Site: [%s] shouldHandleEvent? [%s]", order.getCode(), site.getUid(), String.valueOf(should)));
