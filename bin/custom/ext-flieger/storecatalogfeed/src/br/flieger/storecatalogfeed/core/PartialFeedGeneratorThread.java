...
public class PartialFeedGeneratorThread extends AbstractFeedThread {
    ...
    private void buildCatalogXML() {
        try {
            Set<ProductModel> result = getStoreFeedDao().findByPaging(getCatalogVersion(),
                    getXmlTemplate(), getStartIndex(), PRODUCTS_LIMIT);
    ...
    private String getPartialFeedFileName() {
        String pathname = new StringBuilder().append(getPathByStore()).append(getXmlTemplate())
                .append("/").append(FILE_PREFIX).append("_").append(threadNumber).append(".xml")
                .toString();
        return pathname;
    }
