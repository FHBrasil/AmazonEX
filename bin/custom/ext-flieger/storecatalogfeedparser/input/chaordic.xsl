	...
	<xsl:template match="/">
		<rss version="2.0">
			<channel>
				...
				<description>
					<xsl:value-of select="storeDescription" /> Chaordic
				</description>
					...
					<xsl:for-each select="variants/variant">
						<xsl:if test="variant_stock &gt; 0">
							<sku>
								...
								<xsl:if test="variant_previous_price != '' and variant_previous_price != variant_price">
									<old_price>
										<xsl:value-of select="variant_previous_price" />
									</old_price>
								</xsl:if>
								<price>
								...
								<xsl:if test="variant_main_image != ''">
									<images>
										<default>
											<xsl:value-of select="variant_main_image" />
										</default>
									</images>
								</xsl:if>
								<xsl:if test="variant_color != '' and variant_size != ''">
									<specs>
										<color>
											<xsl:value-of select="variant_color" />
										</color>
								...
								<extra_info>
									<hexa_color>
										<xsl:value-of select="variant_hexa_color" />
									</hexa_color>
								</extra_info>
							</sku>
						</xsl:if>
