    ...
    public boolean putFile(File file) {
        ...
        try {
            ...
            for (FtpProfile profile : ftpProfileChain) {
                ...
                System.out.println("The file " + file.getName() + " was stored? "
                        + fClient.storeFile(file.getName(), new FileInputStream(file)));
                System.out.println("FTP Reply string storing file " + file.getName() + " on "
                        + profile.getFtpAddress() + " : " + fClient.getReplyString());
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
