...
public class InvoiceLinesPixiApiImporter extends AbstractPixiApiImporter {
     ...
     */
    @Override
    public List<InvoiceLineResult> importData(List<PixiFunctionParameter> functionParameters)
    ...
    private List<InvoiceLineResult> importInvoiceLines(
        ...
        SOAPMessage request = getPixiSoapApi().buildMessage(PixiFunction.GET_INVOICE_LINES,
                functionParameters);
                    ...
                    for (int j = 0; j < row.getChildNodes().getLength(); j++) {
                        ...
                        switch (currentTag.getNodeName()) {
                            case INVOICE_LINE_KEY:
                                result.setInvoiceLineKey(Integer.parseInt(currentTag
                                        .getTextContent().trim()));
                                break;
                            case ORDER_LINE_KEY:
                                result.setOrderLineKey(Integer.parseInt(currentTag.getTextContent()
                                        .trim()));
                                break;
                            case ARTICLE_NUMBER:
                                ...
                                break;
                            case ARTICLE_NAME:
                                result.setArtName(currentTag.getTextContent().trim());
                                break;
                            case ITEM_TEXT:
                                result.setItemText(currentTag.getTextContent().trim());
                                break;
                            case ORDER_QUANTITY:
                                result.setOrderQty(Integer.parseInt(currentTag.getTextContent()
                                        .trim()));
                                break;
                            case ITEM_QUANTITY:
                                result.setItemQty(Integer.parseInt(currentTag.getTextContent()
                                        .trim()));
                                break;
                            case ITEM_PRICE:
                                result.setItemPrice(Double.parseDouble(currentTag.getTextContent()
                                        .trim()));
                                break;
                            case SHIPPING_COST:
                                result.setShipCost(Double.parseDouble(currentTag.getTextContent()
                                        .trim()));
                                break;
                            case ITEM_VAT:
                                result.setItemVAT(currentTag.getTextContent().trim());
                                break;
                            case ITEM_NUMBER:
                                result.setItemNr(currentTag.getTextContent().trim());
                                break;
                            case ITEM_NUMBER_SUPPLIER:
                                result.setItemNrSuppl(currentTag.getTextContent().trim());
                                break;
                            case EAN_UPC:
                                result.setEanUpc(currentTag.getTextContent().trim());
                                break;
                            case ORDER_NUMBER:
                                result.setOrderNr(currentTag.getTextContent().trim());
                                break;
                            case ORDER_NUMBER_EXTERNAL:
                                result.setOrderNrExternal(currentTag.getTextContent().trim());
                                break;
                            case STATUS:
                                result.setStatus(currentTag.getTextContent().trim());
                                break;
                            case ITEM_PRICE_NO_VAT:
                                result.setItemPriceNoVAT(Double.parseDouble(currentTag
                                        .getTextContent().trim()));
                                break;
                            case FULL_PRICE:
                                result.setFullPrice(Double.parseDouble(currentTag.getTextContent()
                                        .trim()));
                                break;
                            case DISCOUNT_VALUE:
                                result.setDiscountValue(Double.parseDouble(currentTag
                                        .getTextContent().trim()));
                                break;
                            case DISCOUNT_PERC:
                                result.setDiscountPerc(Double.parseDouble(currentTag
                                        .getTextContent().trim()));
                                break;
                            case ITEM_NUMBER_EXTERNAL:
                                result.setItemNrExtenal(currentTag.getTextContent().trim());
                                break;
                            case ITEM_NUMBER_INTERNAL:
                                result.setItemNrInt(currentTag.getTextContent().trim());
                                break;
                        }
                    }
                    results.add(result);
