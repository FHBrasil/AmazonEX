 ...
 * @author jfelipe
 */
public class InvoicePixiApiImporter extends AbstractPixiApiImporter {

    private static final Logger LOG = Logger.getLogger(InvoicePixiApiImporter.class);
    ...
    public List<InvoiceResult> importData(List<PixiFunctionParameter> functionParameters)
        ...
        return importInvoice(functionParameters);
    }
     ...
     * @author jfelipe
     */
    private List<InvoiceResult> importInvoice(List<PixiFunctionParameter> functionParameters)
            throws SOAPResponseErrorException, SOAPException {
        List<InvoiceResult> results = new ArrayList<InvoiceResult>();
        checkValidParameters(functionParameters);
        SOAPMessage request = getPixiSoapApi().buildMessage(PixiFunction.GET_INVOICES,
                functionParameters);
                    ...
                    for (int j = 0; j < row.getChildNodes().getLength(); j++) {
                        ...
                        switch (currentTag.getNodeName()) {
                            ...
                            case PAID:
                                ...
                                break;
                            case TOTAL_TO_PAY:
                                double totalToPay = Double.parseDouble(currentTagValue);
                                result.setTotalToPay(totalToPay);
                                break;
                            case PAID_SUM:
                                double paidSum = Double.parseDouble(currentTagValue);
