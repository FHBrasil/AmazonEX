...
public class ItemDeliveryInfoPixiApiImporter extends AbstractPixiApiImporter {
    ...
    private static final Logger LOG = Logger.getLogger(ItemDeliveryInfoPixiApiImporter.class);
    //
     ...
     * @return
     *         A List of {@link ItemDeliveryInfoResult} containing the results from Pixi
     *         API
     ...
     */
    @Override
    public List<ItemDeliveryInfoResult> importData(List<PixiFunctionParameter> functionParameters)
    ...
    private List<ItemDeliveryInfoResult> importItemDeliveryInfos(
        ...
        SOAPMessage request = getPixiSoapApi().buildMessage(PixiFunction.GET_ITEM_DELIVERY_INFO,
                functionParameters);
                    ...
                    for (int j = 0; j < rowTag.getChildNodes().getLength(); j++) {
                        ...
                        switch (currentTag.getNodeName()) {
                            ...
                            case SUPPLIER_ORDER_QUANTITY:
                                int supplierorderQuantity = Integer.parseInt(currentTagValue);
