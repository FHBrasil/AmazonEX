...
public class SOrderLinesPixiApiImporter extends AbstractPixiApiImporter {
    ...
    private static final Logger LOG = Logger.getLogger(SOrderLinesPixiApiImporter.class.getName());
    //
     ...
     * tags accordingly to the given list of SOrderKey<br/>
     * 
     ...
     */
    @Override
    public List<SOrderLineResult> importData(List<PixiFunctionParameter> functionParameters)
    ...
    private List<SOrderLineResult> importSOrderLines(
        ...
        if (response != null) {
            Node responseXml = response.getSOAPBody().getElementsByTagName("SqlRowSet1").item(0);
            if (responseXml != null) {
                    ...
                    for (int j = 0; j < rowTag.getChildNodes().getLength(); j++) {
                        ...
                        switch (currentTag.getNodeName()) {
                            ...
                            case ARTICLE_NAME:
                                ...
                                break;
                            case ORDER_QUANTITY:
                                int orderQuantity = Integer.parseInt(currentTagValue);
                            ...
                            case S_ORDER_LINE_KEY:
                                int sOrderLineKey = Integer.parseInt(currentTagValue);
                                result.setSOrderLineKey(sOrderLineKey);
                                break;
                            case S_ITEM_REFERENCE:
                                int sItemReference = Integer.parseInt(currentTagValue);
                                result.setSItemReference(sItemReference);
                                break;
                            case S_ORDER_REFERENCE:
                                int sOrderReference = Integer.parseInt(currentTagValue);
                                result.setSOrderReference(sOrderReference);
                                break;
                            case ML_CONFIRM:
                                boolean mlConfirm = Boolean.valueOf(currentTagValue).booleanValue();
                                result.setMlConfirm(mlConfirm);
                                break;
                            case S_ORDER_LINE_NUMBER:
                                int sOrderLineNumber = Integer.parseInt(currentTagValue);
                            ...
                            case ESTIMATED_DELIVERY:
                                ...
                                break;
                            case VK_PRICE:
                                double vkPrice = Double.parseDouble(currentTagValue);
                                result.setVkPrice(vkPrice);
                                break;
                            case SUMMARY_QUANTITY:
                                int summaryQuantity = Integer.parseInt(currentTagValue);
                                result.setSummaryQuantity(summaryQuantity);
                                break;
                            case LOCATION_QUANTITY:
                                int locationQuantity = Integer.parseInt(currentTagValue);
                                result.setLocationQuantity(locationQuantity);
                                break;
                            case QUANTITY_RECEIVED:
                                int quantityReceived = Integer.parseInt(currentTagValue);
                                result.setQuantityReceived(quantityReceived);
                                break;
                            case QUANTITY_NOT_DELIVERED:
                                int quantityNotDelivered = Integer.parseInt(currentTagValue);
                                result.setQuantityNotDelivered(quantityNotDelivered);
                                break;
                            case MIN_ORDER_QUANTITY:
                                int minOrderQuantity = Integer.parseInt(currentTagValue);
                                result.setMinOrderQuantity(minOrderQuantity);
                                break;
                            case MIN_STOCK_QUANTITY:
                                int minStockQuantity = Integer.parseInt(currentTagValue);
                            ...
                            case ITEM_NUMBER_INTERNAL:
                                ...
                                break;
                            case VAT_PERC:
                                double vatPerc = Double.parseDouble(currentTagValue);
                                result.setVatPerc(vatPerc);
                                break;
                            case EK_PRICE:
                                double ekPrice = Double.parseDouble(currentTagValue);
                                result.setEkPrice(ekPrice);
                                break;
                            case EK_VALUE:
                                double ekValue = Double.parseDouble(currentTagValue);
                            ...
                            case OPEN_SUPPLIER_0:
                                int openSupplierO = Integer.parseInt(currentTagValue);
                            ...
                            case L_365:
                                ...
                                break;
                            case STOCK_QUANTITY:
                                int stockQuantity = Integer.parseInt(currentTagValue);
