 ...
 * @author jfelipe
 *
 * @deprecated Use this test you must not! By Herr Gleichmann words, we
 *             shouldn't update stock data on Pixi.
 *
 */
// @UnitTest
@Deprecated
public class StockPixiApiExporterTest {
	...
	public void importDataTest() {
		try {
			int stockQuantity = new Random(Calendar.getInstance()
					.getTimeInMillis()).nextInt(200);
			//
			...
			String formattedTestDate = new SimpleDateFormat(
					"yyyy-MM-dd HH:mm:ss").format(testDate.getTime());
			...
			Set<ItemStockResult> results = stockImporter
					.importData(parametersImporter);
			...
			Assert.assertEquals(
					"Stock quantity received should be the same as sent.",
	...
	public void importData_EanNotExists() {
		try {
			...
			Assert.assertEquals("Wrong exception type.",
					SOAPResponseErrorException.class.getName(), re.getClass()
							.getName());
		} catch (Exception e) {
			Assert.assertTrue("Should not have thrown an exception. Type: "
					+ e.getClass().getName() + ". Message: " + e.getMessage(),
